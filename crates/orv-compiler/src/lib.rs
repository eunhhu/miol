...
fn orv_native_handle_connection(stream: &mut std::net::TcpStream) -> std::io::Result<()> {{
    ...
    let dispatch = router::orv_native_dispatch(&request.method, &request.path);
...
fn native_server_direct_http_capable(artifact: &ServerRuntimeArtifact) -> bool {
    artifact
        .routes
        .iter()
        .all(native_server_route_has_static_response)
}

fn native_server_route_has_static_response(route: &ServerRouteArtifact) -> bool {
...
@out count"#,
  ...
  @route GET /ping {
    @respond 200 { ok: true }
        ...
        assert!(source.contains("router::orv_native_dispatch(&request.method, &request.path)"));
