...
fn native_server_response_is_direct(
    ...
    response
        ...
        && (response.body_kind == "empty"
            ...
            || native_server_response_uses_query_params(response)
    ...
    !response.body_object_fields.is_empty()
        && response
            ...
            .all(|field| match field.value_kind.as_str() {
                ...
                "route_param" | "route_param_int" | "route_param_float" => field
                    .name
                    .as_ref()
...
fn native_server_response_uses_query_params(response: &ServerResponseArtifact) -> bool {
...
impl<'a> NativeCapturedJsonOperation<'a> {
    fn none() -> Self {
        Self {
            op: None,
            operand_json: None,
...
fn push_native_int_success_arm(
    ...
    match (op, operand_json, operand_kind, operand_name) {
        ...
        (Some("add" | "mul"), Some(operand_json), None, None) => {
        ...
        (Some("add" | "mul"), None, Some("request_body_field_int"), Some(operand_name)) => {
            let method = if op == Some("mul") {
                "checked_mul"
            } else {
                "checked_add"
            ...
            let _ = writeln!(
                source,
                "            Ok(value) => match routes::orv_native_body_field_value(route_match, {}).unwrap_or(\"\").trim().parse::<i64>() {{",
    ...
    match &expr.kind {
        HirExprKind::Object(fields) | HirExprKind::TypedObject { fields, .. } => {
            ...
            for field in fields {
                ...
                let (param, value_kind) = route_param_field_value(&field.value)?;
...
fn route_param_field_value(expr: &HirExpr) -> Option<(String, String)> {
    match &expr.kind {
        HirExprKind::Cast { expr, ty } if is_integer_type_ref(ty) => {
            Some((route_param_field_name(expr)?, "route_param_int".to_string()))
        }
        HirExprKind::Cast { expr, ty } if is_float_type_ref(ty) => Some((
            route_param_field_name(expr)?,
            "route_param_float".to_string(),
        )),
        HirExprKind::Paren(expr) => route_param_field_value(expr),
        _ => Some((route_param_field_name(expr)?, "route_param".to_string())),
    }
}
...
fn request_body_field_value(expr: &HirExpr) -> Option<CapturedResponseValue> {
    match &expr.kind {
        HirExprKind::Binary { op, lhs, rhs } if matches!(op, BinaryOp::Add | BinaryOp::Mul) => {
            ...
            let operand = request_body_field_value(rhs)?;
            if operand.value_kind != "request_body_field_int" || operand.op.is_some() {
                return None;
            }
