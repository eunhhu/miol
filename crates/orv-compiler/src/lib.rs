...
/// Generate Rust source for the reference native server launcher bridge.
///
/// This source is still a reference launcher, not final native codegen. It
/// verifies that the native plan and server runtime artifact exist, links the
/// generated route table module, and shells through `orv run-artifact`.
#[must_use]
...
pub fn orv_native_dispatch(method: &str, path: &str) -> OrvNativeDispatch {
    if let Some(route_match) = routes::orv_native_match_route(method, path) {
...
@out count"#,
  ...
  @route GET /assets/:rest* {
    ...
    #[test]
    fn native_server_launcher_source_declares_reference_bridge() {
