...
Runtime commerce adapters currently provide local reference handles: `@payment.connect("test://local").capture(...)` returns captured payment metadata and `@shipping.connect("test://local").book(...)` returns shipment booking metadata. The shop scaffold uses file-backed local handles, `@payment.connect("file://data/payments.jsonl")` and `@shipping.connect("file://data/shipments.jsonl")`, so capture/booking records are appended and synced to local JSONL files before matching DB rows are persisted. `http://host/path` handles are reference external adapters: capture/book calls POST `{"kind","payload"}` JSON to the configured HTTP endpoint and return its JSON response as an ORV value. Prod deploy persistence records relative file adapter paths as `record_paths` and mounts their parent directories into Compose; HTTP adapter URLs are recorded as `commerce_endpoints` in deploy/container persistence and listed in the runbook without adding volumes. When the adapter URL comes from `@env.NAME ?? "http://default"`, deploy/container persistence also records `commerce_env` with the env name and default URL, the generated Compose file forwards that env with the default fallback, and the runbook lists the override knob. Builds that use these connections record `payment_adapter` and `shipping_adapter` runtime features in build, server runtime, and deploy artifacts. Provider-specific payment/shipping adapter URLs such as `stripe://` and `carrier://` are rejected until dedicated provider integrations exist, so the scaffold does not silently pretend to support live provider SDKs.
