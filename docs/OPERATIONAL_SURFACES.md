...
`orv editor snapshot <file>` emits first-party editor bootstrap JSON with diagnostics, ProjectGraph, Files/Routes/Schema/Domains panel inputs, and source-hash watch sources. `orv editor reveal <dir> <origin-id>` maps build artifact origins to editor focus/source/production navigation payloads. `orv editor runtime <file>` reuses DAP trace/runtime helpers for runtime inspection pane JSON. `orv editor export <file> --out <dir>` writes `state.json` plus a static `index.html` shell with panel lists, ProjectGraph visualization, runtime frame inspection, DAP adapter launch/live/attach configuration wiring, live-control request payloads, executable breakpoint source lines, and optional trace navigation. `orv editor trace <dir> --trace <trace.json>` maps captured request frames back to source/production navigation and embeds the `/__orv/trace/events` EventSource transport URL when a server artifact has a stable listen endpoint.
