...
| Template-to-running-shop smoke path | implemented | experimental | M2 | `orv-cli`, `orv-runtime` | generated smoke-test | `orv init`, `orv build --prod`, smoke-test | First acceptance target before human 5h runs; generated prod smoke now checks route reachability, CSRF/session/admin cookies, checkout response markers, and customer/admin read-model body markers |
