...
| PostgreSQL/MySQL adapters | reference stub | reference | M4+ | `orv-runtime`, `orv-cli` | external DB adapter bridge runtime/deploy artifact/env-check tests | `orv run`, `orv build --prod`, `orv deploy-env-check` | Default handles expose explicit unsupported status/fail query methods; when `ORV_DB_ADAPTER_POSTGRES_ENDPOINT`, `ORV_DB_ADAPTER_MYSQL_ENDPOINT`, or `ORV_DB_ADAPTER_ENDPOINT` is configured, query methods POST checked `http-json-v1` requests to the external bridge and return its JSON response. Prod artifacts expose the bridge request shape plus provider-specific endpoint/auth env knobs, and deploy env check requires provider-specific bridge endpoints before launch. Direct provider drivers remain planned |
...
| Template-to-running-shop smoke path | implemented | experimental | M2 | `orv-cli`, `orv-runtime` | generated smoke-test | `orv init`, `orv build --prod`, smoke-test | First acceptance target before human 5h runs; generated prod smoke now checks route reachability, exact `x-orv-origin-id` route headers, CSRF/session/admin cookies, checkout response markers, and customer/admin read-model body markers |
