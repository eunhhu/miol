...
| Typed body/form validation | implemented | experimental | M1 | `orv-syntax`, `orv-runtime` | request binding runtime tests | `orv run` | `@body: T`, `@query: T`, `@form: T` named-schema bindings use runtime validators, normalize request-state values, and return 400 `validation_failed` payloads on failure |
