...
| Auth/member session scaffold | reference stub | reference | M2 | `orv-cli`, `orv-runtime` | `fixtures/e2e/shopping_mall.orv` | `orv init`, `orv run` | Member/session rows exist, successful login emits an `orv_session` cookie with HttpOnly/SameSite/Secure defaults, and `@session required` gates cookie-backed routes; declarative `@Auth` hardening still needed |
