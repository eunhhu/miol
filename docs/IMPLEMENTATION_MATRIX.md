...
| `orv reveal` / editor reveal payload | implemented | experimental | M3 | `orv-cli`, `orv-compiler` | reveal CLI tests | `orv reveal`, `orv editor reveal` | Build origin to source/production payload, including graph-contract and preflight smoke evidence summary counters |
...
| Static editor export | implemented | experimental | M3 | `orv-cli` | editor export tests | `orv editor export` | Graph/panel/trace HTML artifacts; production export mirrors source-bundle/project-graph/origin-map graph contract and preflight smoke evidence summary counts into `state.json`, `native-host.json`, and `production/panel.html` |
