...
- **문서 구조 가이드 — `docs/DOCUMENTATION.md`**: 비전 문서, 확정 사양, 실험 사양, 구현 문서의 역할과 수정 규칙을 정리한다.
...
공식 언어 사양은 [SPEC.md](SPEC.md)에 정리되어 있고, `fixtures/plan/`과 `default-syntax.orv`는 그 사양을 압박 테스트하거나 미래 방향을 탐색하는 실험 예제 역할을 한다. 컴파일러 크레이트 구현은 진행 중이며, 현재 구조는 [ARCHITECTURE.md](ARCHITECTURE.md)를 참조.
