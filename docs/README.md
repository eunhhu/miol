...
Workspace package 세부: `orv workspace lock [root] --out <dir>`은 workspace graph/path dependency edge를 재사용해 member를 dependency-first 순서로 lock하고, `members/<member>/orv.lock`과 top-level `workspace-lock.json` artifact를 출력한다. `orv workspace fetch [root] --out <dir>`은 그 lock artifact를 member별 dependency cache로 materialize하고 `workspace-fetch.json`을 출력한다.
