...
Package lock 세부: `orv lock`은 prerelease/build metadata 포함 exact version을 그대로 고정하고, local/file/HTTP registry `index.json`이 있으면 `*`, `x`, segment wildcard, caret, tilde, whitespace-AND comparator, `||` disjunction range를 exact version으로 해석하며 원래 range는 `requested_version`으로 보존한다. HTTPS/authenticated registry transport는 로드맵이다.
