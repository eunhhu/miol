...
- **프로덕션-코드 양방향 추적**: 부분 구현 — HIR origin map과 HTTP route origin id 헤더 제공. 화면, DB 쿼리, job, 로그에서 원본 코드와 도메인 노드로 reveal하는 기능은 로드맵
...
공식 언어 사양은 [SPEC.md](SPEC.md)에 정리되어 있고, `fixtures/plan/`과 `default-syntax.orv`는 그 사양을 압박 테스트하거나 미래 방향을 탐색하는 실험 예제 역할을 한다. 현재 실행 표면은 `orv run/check/dump/origins`와 레퍼런스 인터프리터 중심이며, 서버/WASM 번들링 컴파일러는 로드맵이다. 현재 구조는 [ARCHITECTURE.md](ARCHITECTURE.md)를 참조.
