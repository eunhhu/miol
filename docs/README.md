...
Native request-body scaled arithmetic 보충: `(@body.quantity as int) + ((@body.bonus as int) * 2)`처럼 captured int/float에 captured operand와 static scale 곱을 더하거나 빼는 response field도 `add_scaled`/`sub_scaled` metadata로 낮춰 generated handler에서 checked multiply/add/sub로 처리한다.
