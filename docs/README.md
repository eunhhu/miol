...
Editor DAP native control 세부: exported debug controls now include exact per-control runner commands, executable breakpoint rows include `--breakpoint <path>:<line> --control continue` runner commands and `setBreakpoints` request previews, `native-host.json` mirrors launch/live/attach configurations, controls, breakpoint command inventory, breakpoint argument format, and `debug/session-result.json` result artifact with `panels.debug`, and the static shell shows the selected runner command plus result artifact metadata beside the DAP request payload.
