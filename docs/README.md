...
마지막 unconditional `@respond`를 `else` 안에 넣지 않고 route handler의 다음 stmt로 둔 `if ... else if ...; @respond ...` 형태도 같은 ordered response artifact로 직접 dispatch한다.

`else { if ... }`처럼 single nested `if`를 감싸는 block도 동일한 ordered response artifact로 정규화한다.

Route path param matcher는 `:id`와 `:rest*`뿐 아니라 `:id.json` 같은 static suffix segment도 캡처하며, analyzer는 route path에 없는 `@param.<name>` 참조를 build/check 전에 진단한다.

Client bundle 보충: interactive client build는 `client/manifest.json`과 `client/reactive-plan.json`도 출력한다. manifest는 `pages/index.html`, reactive plan, `client/app.js`, `client/app.wasm` path/hash, `source-bundle.json` stable hash, WASM exports, initial render metadata, `client_wasm` runtime feature, `dynamic-client-codegen`/`reactive-dom-diff` blockers를 하나로 묶고, prod `deploy/manifest.json`은 이를 `client.manifest`로 참조한다. reactive plan은 source-backed `let sig` origin, top-level signal initial state metadata, checked initial-render binding from `client/app.wasm` to `pages/index.html`, per-signal `signal_state` binding to `client/app.js`, source-backed `signal_text` binding to initial DOM text slots and interpolated text templates such as `@p "count: {count}"` plus multi-signal templates such as `@p "{label}: {count}"` with explicit `state_keys`/`sources`, conditional text such as `@p { count > 0 ? "has items" : "empty" }`, source-backed `signal_attr` binding to initial DOM attributes/properties such as `@input value={input}`, single-signal interpolated attribute templates such as `@input placeholder="{input}!"`, multi-signal attribute templates such as `@input placeholder="{prefix}-{count}"`, single-signal boolean conditional attributes such as `class={active ? "enabled" : "disabled"}`, and single-signal literal comparison attributes such as `class={count > 0 ? "enabled" : "disabled"}`, source-backed `signal_event` binding for simple event-time signal assignments such as `onClick={count += 1}`, boolean toggles such as `onClick={muted = !muted}`, form input value updates such as `onInput={(e) -> email = e.target.value}`, checkbox checked updates such as `onChange={(e) -> accepted = e.target.checked}`, and numeric input casts such as `float.from(e.target.value)`/`int.from(e.target.value)`, source hash, future full DOM diff blocker를 고정한다. 생성된 `client/app.js`는 manifest와 reactive plan을 먼저 fetch해 schema/source hash/WASM path/hash/export/reactive-plan/signal-state/signal-text/signal-attr/signal-event binding 계약을 검증한 뒤 source bundle과 WASM을 로드하고, decoded WASM initial render hash/byte length를 manifest `initial_render`와 대조한 뒤 text slot/template/condition과 attr value/template/condition을 cursor-stable duplicate initial slot matching으로 붙이고 event update hook을 붙여 `__ORV_SET_SIGNAL__`을 노출하고 client root에 reactive signal count, binding count, DOM binding count, attr binding count, event binding count, state hash를 기록한다. `orv verify-build`는 manifest/reactive plan/page/loader/WASM/source bundle/WASM-hash/initial-render/reactive-state loader contract 일치를 검증하고, `orv reveal`/`orv editor reveal`/`orv lsp reveal`은 client origin production payload에 manifest/reactive plan summary와 `wasm_hash`를 노출한다.

DB PITR 세부: `orv db restore --archive <archive.json> --data <data.json> --at <RFC3339>`는 hash-verified WAL archive manifest를 지정 시점까지 재생해 `@db.save` 호환 snapshot으로 복원한다. archive manifest의 relative source WAL path는 현재 cwd가 아니라 manifest 파일 디렉터리 기준으로 해석된다. `orv db archive --target http://host/archive`는 WAL과 manifest를 HTTP `POST`로 업로드하고 local manifest에 remote WAL/manifest URL을 기록한다. `orv db archive --target s3://bucket/prefix`는 `ORV_DB_ARCHIVE_S3_ENDPOINT=http(s)://host`를 S3-compatible endpoint로 사용해 WAL/manifest를 `PUT`으로 업로드하고, `ORV_DB_ARCHIVE_S3_AUTH_TOKEN` bearer 또는 `ORV_DB_ARCHIVE_S3_AUTH=aws-sigv4` with `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_REGION`를 upload/restore request에 붙인다. source WAL이 없으면 archive restore/recover가 HTTP/S3-compatible target WAL을 `GET`으로 내려받아 hash/byte 검증 후 복원한다. HTTP/S3-compatible upload/download는 기본 3회 transient retry를 수행하고 `ORV_DB_ARCHIVE_HTTP_RETRY_ATTEMPTS=1..8`로 조정할 수 있다. `orv db crash-matrix --out <report.json>`은 로컬 WAL crash/recovery 시나리오를 실행해 운영 검증용 pass/fail report를 만든다.

DB adapter 세부: reference runtime의 `@db.connect`는 `memory://` handle, local WAL-backed `file://path` handle, 실제 SQLite 파일에 ORV row JSON을 저장/재로드하는 `sqlite://path` handle을 허용하고, PostgreSQL/MySQL adapter URL은 실제 adapter 구현 전까지 명시적으로 실패한다. PostgreSQL/MySQL status handle은 `adapterStatus: "unsupported_runtime"`와 `runtime.queryMethods` metadata를 함께 노출한다. `@db.connect` 사용 build/deploy/server runtime artifact는 `db_adapter` runtime feature를 기록한다. Prod deploy persistence는 `@db.wal("data/app.wal.jsonl")`, `@db.connect "file://data/app.wal.jsonl"`, `@db.connect "sqlite://data/app.sqlite"`를 찾아 WAL은 `wal_paths`, SQLite 파일은 `db_paths`, PostgreSQL/MySQL URL은 `db_endpoints`에 기록하고, local file/SQLite path에만 Compose volume을 만든다. Prod build는 `deploy/db-adapters.json`도 써서 PostgreSQL/MySQL adapter provider, env/default, endpoint, 현재 `unsupported_runtime` 상태와 reference query method contract를 검증 가능한 운영 산출물로 노출하고, `orv verify-build`가 source bundle 재분석 결과와 drift를 검사한다. Env-only DB adapter는 required deploy env로 취급되어 `orv deploy-env-check <dir>`이 unset/blank 값을 배포 전 실패로 보고한다.

Commerce adapter 세부: reference runtime은 local adapter로 `@payment.connect("test://local").capture(...)`와 `@shipping.connect("test://local").book(...)`을 제공하고, shop scaffold는 기본적으로 `@payment.connect(@env.PAYMENT_ADAPTER_URL ?? "file://data/payments.jsonl")`와 `@shipping.connect(@env.SHIPPING_ADAPTER_URL ?? "file://data/shipments.jsonl")`을 사용한다. env가 없으면 `file://path.jsonl` commerce adapter URL은 capture/booking record를 local JSONL 파일에 append+sync 하고, env override로 지정한 `http://host/path` commerce adapter URL은 `capture`/`book` 호출을 `{"kind","payload"}` JSON POST로 외부 HTTP endpoint에 전달하고 응답 JSON을 ORV 값으로 반환한다. Prod deploy persistence는 relative file adapter path를 `record_paths`로 기록하고 Compose volume을 만들며, static HTTP commerce adapter endpoint는 `commerce_endpoints`로 기록한다. `@payment.connect(@env.PAYMENT_ADAPTER_URL ?? "http://host/capture")`나 shipping 대응 형태처럼 env-configured adapter는 env/default 쌍을 `commerce_env`로 기록하고 generated Compose에 `${VAR:-default}` 환경 변수를 주입하며 runbook에 operator override를 함께 나열한다. HTTP default는 `commerce_endpoints`, file default는 `record_paths`에도 기록된다. HTTP adapter는 volume을 만들지 않는다. Prod build는 `deploy/commerce-adapters.json`도 써서 payment/shipping adapter kind, mode, env/default, endpoint 또는 record path, HTTP POST JSON request contract를 검증 가능한 운영 산출물로 노출한다. 해당 연결을 쓰는 build/deploy/server runtime artifact는 `payment_adapter`/`shipping_adapter` runtime feature를 기록한다. `stripe://` payment URL과 `carrier://` shipping URL은 provider mode reference adapter로 동작하고 `deploy/commerce-adapters.json`에 provider 이름과 provider endpoint/credential env contract를 기록한다. Provider-mode deploy artifacts, Compose, env.example, and runbook expose `STRIPE_API_ENDPOINT`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `CARRIER_API_ENDPOINT`, `CARRIER_API_KEY`, and `CARRIER_WEBHOOK_SECRET` placeholders; `orv deploy-env-check <dir>` fails missing required provider credentials before launch, runtime responses report configured/missing status without exposing the secret values, configured provider endpoint calls POST checked JSON with bearer credentials, stable idempotency keys, and bounded transient retry, then merge provider JSON responses, Stripe handles expose `payments.verifyWebhook({ payload, signature })` for reference HMAC-SHA256 webhook signature checks, and the shop scaffold hosts `POST /webhooks/stripe` to verify signatures, persist `WebhookEvent.eventId`, return the existing row for duplicate provider/eventId deliveries, and reconcile Payment/Order statuses from verified webhook payload fields. Production provider SDK matrix/credential rotation hardening remains future work.
...
LSP bootstrap은 `declaration`/`typeDefinition` navigation, project moniker, document colors, linked editing, type hierarchy, function call `signatureHelp`, parameter `inlayHint`, document/range/on-type formatting, context-aware completion snippets with domain/env dot completions with route-param inference and domain-field hover descriptions, route-head edit quickfixes, guarded identifier edit surfaces, save/close lifecycle cache까지 제공한다. DAP bootstrap은 현재 ProjectGraph/AST 기반 verified nested line breakpoint/goto target line, conditional/hit-condition breakpoint, non-stopping `logMessage` logpoint, function breakpoint, instruction breakpoint unverified response, source-frame pseudo `disassemble`, source-frame `readMemory`, data breakpoint, ORV diagnostics/runtime exception filters, source/module inventory와 snapshot-backed `source` content/checksum, debug-control thread id guard, reference-runtime debug frame capture, `launch.arguments.live=true` DebugStepper 기반 incremental runtime frame 실행, standard `attach` request를 통한 runtime attach, 장기 실행 `@server` launch의 non-blocking paused frame과 `continue`/`pause` event loop, `supportsOrvRuntimeAttach`/`supportsOrvRuntimeTracePath`/`supportsDisassembleRequest`/`supportsReadMemoryRequest` initialize capability, `launch.arguments.attachRuntime=true` child-process accept-loop attach, `attachRuntimeMode="inProcess"` in-process accept-loop attach, process STOP/CONT 또는 in-process shutdown/restart pause/resume, launch runtime JSON/variables/evaluate/completions async state/static/env listen endpoint/route inventory/transport process/address inspect 노출, in-process request count/last request/request frame/request trace JSON inspect와 `runtimeRequestTracePath` file flush 노출, runtime-owned `orv.production.trace` schema/file writer 재사용, `ORV_RUNTIME_REQUEST_TRACE_PATH` graceful server trace file capture, `/__orv/trace/events` open-ended live trace EventSource snapshot/frame event, runtime-evaluated Locals scope, captured function call-stack `stackTrace`, reported frame-id `scopes`/`stepInTargets`/`restartFrame`, current-frame local-name `evaluate`/`completions`, Locals snapshot `setVariable`/`setExpression`, runtime-frame `next` step-over, captured `stepIn.targetId`, live targetId 거부 `stepIn`, `stepOut`, `stepBack`, `goto` progression with last-frame termination, next line/function/data breakpoint `continue`/`reverseContinue`, per-frame stdout `output` events, `pause` stopped event, live-mode 보존 `restart`, stderr `output` events, stdio `initialized`/`stopped`/`continued`/`terminated` events까지 제공한다. `orv editor snapshot`은 first-party editor Files/Routes/Schema/Domains 패널 입력과 source-hash live refresh watch set을 만들고, `orv editor reveal`은 build origin id를 editor focus/source/production navigation payload로 바꾸며, `orv editor runtime`은 runtime status/stdout/frames를 editor pane JSON으로 만든다. `orv editor debug`는 optional `--breakpoint <path>:<line>` setBreakpoints와 live DAP control sequence를 Content-Length transport 한 session에서 왕복한 response/event frame JSON으로 만든다. `orv editor run-debug`는 exported `debug.session_runner` state 또는 `debug/session-runner.json` standalone artifact를 같은 DAP path로 실행하고 optional breakpoint를 적용하는 native host runner result JSON을 stdout과 `debug/session-result.json` artifact/`debug/session-result.html` panel로 만들며 `panels.debug.session_summary`, `panels.debug.source_navigation`, `panels.debug.controls`, `panels.debug.breakpoints`, selected-frame/stack/scopes/project variables/locals/event summary를 포함한다. `orv editor export`는 이 state, `debug/session-runner.json`, `native-host.json`를 static HTML shell과 함께 출력하고 ProjectGraph view, panel list, DAP launch config/breakpoint line and runner commands, debug result artifact metadata, trace summary, trace stream runner command, selectable runtime/trace detail, optional trace navigation state도 embed한다. `native-host.json`은 native UI가 필요한 shell/state/runner/result panel/adapter/breakpoint/trace-stream/capability 경로를 한 JSON에서 찾게 한다. `orv editor trace`는 captured request trace navigation과 stable server EventSource trace transport metadata를 제공한다. `orv editor trace-stream`은 native host가 받은 EventSource snapshot/frame body를 editor trace stream JSON으로 정규화한다. richer native trace UI와 richer native UI는 다음 단계다.

Editor DAP/native trace control 세부: exported debug controls now include exact per-control runner commands, executable breakpoint rows include `--breakpoint <path>:<line> --control continue` runner commands and `setBreakpoints` request previews, `native-host.json` mirrors launch/live/attach configurations, controls, breakpoint command inventory, breakpoint argument format, `debug/session-result.json` result artifact with `panels.debug.session_summary`, `panels.debug.source_navigation`, `panels.debug.scopes`, locals/project variables, `panels.debug.controls`, and `panels.debug.breakpoints` plus its section `panel_contract`, `trace.summary` with trace path/frame/status/first-last request summaries, and the rendered `debug/session-result.html` companion panel, and the static shell shows the selected runner command plus result artifact metadata beside the DAP request payload.
