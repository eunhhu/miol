...
Client bundle 보충: interactive client build는 `client/manifest.json`과 `client/reactive-plan.json`도 출력한다. manifest는 `pages/index.html`, reactive plan, `client/app.js`, `client/app.wasm`, `source-bundle.json` stable hash, WASM exports, initial render metadata, `client_wasm` runtime feature, `dynamic-client-codegen`/`reactive-dom-diff` blockers를 하나로 묶고, prod `deploy/manifest.json`은 이를 `client.manifest`로 참조한다. reactive plan은 source-backed `let sig` origin, checked initial-render binding from `client/app.wasm` to `pages/index.html`, source hash, future DOM diff blocker를 고정한다. 생성된 `client/app.js`는 manifest와 reactive plan을 먼저 fetch해 schema/source hash/WASM/export/reactive-plan/binding 계약을 검증한 뒤 source bundle과 WASM을 로드하고, decoded WASM initial render hash/byte length를 manifest `initial_render`와 대조한 뒤 client root에 reactive signal count를 기록한다. `orv verify-build`는 manifest/reactive plan/page/loader/WASM/source bundle/initial-render loader contract 일치를 검증하고, `orv reveal`/`orv editor reveal`/`orv lsp reveal`은 client origin production payload에 manifest/reactive plan summary를 노출한다.
