...
DB adapter 세부: reference runtime의 `@db.connect`는 `memory://` handle과 local WAL-backed `file://path` handle을 허용하고, 그 밖의 외부 adapter URL은 실제 adapter 구현 전까지 명시적으로 실패한다. Prod deploy persistence는 `@db.wal("data/app.wal.jsonl")`와 `@db.connect "file://data/app.wal.jsonl"`를 모두 찾아 Compose volume을 만든다.
