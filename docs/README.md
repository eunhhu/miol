...
DAP bootstrap은 현재 ProjectGraph/AST 기반 verified breakpoint line, ORV diagnostics/runtime exception filters, reference-runtime debug frame capture, runtime-evaluated Locals scope, current-frame local-name `evaluate`/`completions`, runtime-frame `next`/`stepIn`/`stepOut` progression with last-frame termination, next-breakpoint `continue`, `pause` stopped event, `restart`, stdout/stderr `output` events, stdio `initialized`/`stopped`/`continued`/`terminated` events까지 제공한다. 실제 장기 실행 런타임 중단, call-stack frame hierarchy, incremental output delivery는 다음 단계다.
