...
Native request-body scaled arithmetic 보충: `(@body.quantity as int) + ((@body.bonus as int) * 2)`, `((@body.bonus as int) * 2) + (@body.quantity as int)`, `(@body.quantity as int) - ((@body.bonus as int) * 2)`처럼 supported captured int/float와 captured operand static-scale 곱을 더하거나 빼는 response field도 `add_scaled`/`sub_scaled` metadata로 낮춰 generated handler에서 checked multiply/add/sub로 처리한다.
