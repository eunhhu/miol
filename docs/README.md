...
Build/deploy 보충: 서버 build는 `server/native-server.json` native server plan contract, `server/runtime-image.json` native runtime image plan contract, `server/native/Cargo.toml`/`server/native/main.rs` reference launcher package도 출력한다. 이 파일들은 실제 final native binary/image가 아니라 planned `server/app` HTTP/1 target, planned `orv-native-server:latest` OCI image target, 현재 reference artifact/launcher/source/reference image, structured build/run command, route/listen/runtime feature shape, `native-codegen`/`native-runtime-image` blocker를 기록한다. `server/native/main.rs`는 native plan file과 server artifact를 확인한 뒤 `orv run-artifact`를 호출하고 process arguments를 넘기는 생성 Rust source다. `orv build --prod`의 `deploy/manifest.json`은 이를 `server.native_plan`과 `server.native_runtime_image_plan`으로 참조하고, generated deploy runbook은 `cargo build --manifest-path server/native/Cargo.toml --release`와 `ORV_BUILD_DIR=. ./server/native/target/release/orv-native-server` 실행 경로를 문서화하며, shop starter README는 같은 흐름을 `dist/` path로 쓴다. `orv verify-build`는 해당 contract와 runbook 명령이 server runtime artifact와 계속 일치하는지 검증한다. `orv reveal`/`orv editor reveal`/`orv lsp reveal`은 route origin의 production payload에 이 native server target, build/run commands, runtime image plan summary를 함께 노출한다.
