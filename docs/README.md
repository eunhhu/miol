...
DB adapter 세부: reference runtime의 `@db.connect`는 `memory://` handle, local WAL-backed `file://path` handle, 실제 SQLite 파일에 ORV row JSON을 저장/재로드하는 `sqlite://path` handle을 허용하고, PostgreSQL/MySQL adapter URL은 실제 adapter 구현 전까지 명시적으로 실패한다. `@db.connect` 사용 build/deploy/server runtime artifact는 `db_adapter` runtime feature를 기록한다. Prod deploy persistence는 `@db.wal("data/app.wal.jsonl")`, `@db.connect "file://data/app.wal.jsonl"`, `@db.connect "sqlite://data/app.sqlite"`를 찾아 WAL은 `wal_paths`, SQLite 파일은 `db_paths`, PostgreSQL/MySQL URL은 `db_endpoints`에 기록하고, local file/SQLite path에만 Compose volume을 만든다.
