...
DAP bootstrap은 현재 ProjectGraph/AST 기반 verified breakpoint line, ORV diagnostics/runtime exception filters, stopped-line-filtered top-level `let`/`const` Locals scope, pure-expression/array/object local value evaluation, local-name `evaluate`/`completions`, entry-line `next`/`stepIn`/`stepOut` progression with last-line termination, `pause` stopped event, `restart`, stdout/stderr `output` events, stdio `initialized`/`stopped`/`continued`/`terminated` events까지 제공한다. 실제 장기 실행 런타임 중단/동적 expression frame locals는 다음 단계다.
