...
Client bundle 보충: interactive client build는 `client/manifest.json`도 출력한다. 이 manifest는 `pages/index.html`, `client/app.js`, `client/app.wasm`, `source-bundle.json` stable hash, WASM exports, initial render metadata, `client_wasm` runtime feature, `dynamic-client-codegen`/`reactive-dom-diff` blockers를 하나로 묶고, prod `deploy/manifest.json`은 이를 `client.manifest`로 참조한다. 생성된 `client/app.js`는 이 manifest를 먼저 fetch해 schema/source hash/WASM/export 계약을 검증한 뒤 source bundle과 WASM을 로드한다. `orv verify-build`는 manifest와 page/loader/WASM/source bundle 일치를 검증하고, `orv reveal`/`orv editor reveal`/`orv lsp reveal`은 client origin production payload에 manifest summary를 노출한다.
