...
Build/deploy 보충: 서버 build는 `server/native-server.json` native server plan contract도 출력한다. 이 파일은 실제 native binary가 아니라 planned `server/app` HTTP/1 target, 현재 reference artifact/launcher, route/listen/runtime feature shape, `native-codegen`/`native-runtime-image` blocker를 기록한다. `orv build --prod`의 `deploy/manifest.json`은 이를 `server.native_plan`으로 참조하고, `orv verify-build`는 해당 contract가 server runtime artifact와 계속 일치하는지 검증한다. `orv reveal`/`orv editor reveal`/`orv lsp reveal`은 route origin의 production payload에 이 native server target을 함께 노출한다.
