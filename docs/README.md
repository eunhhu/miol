...
DB PITR 세부: `orv db restore --archive <archive.json> --data <data.json> --at <RFC3339>`는 hash-verified WAL archive manifest를 지정 시점까지 재생해 `@db.save` 호환 snapshot으로 복원한다. archive manifest의 relative source WAL path는 현재 cwd가 아니라 manifest 파일 디렉터리 기준으로 해석된다. `orv db archive --target http://host/archive`는 WAL과 manifest를 HTTP `POST`로 업로드하고 local manifest에 remote WAL/manifest URL을 기록한다. source WAL이 없으면 archive restore/recover가 HTTP target WAL을 `GET`으로 내려받아 hash/byte 검증 후 복원한다.
