...
- `orv db archive --wal db.wal.jsonl --out archive.json` — 현재 구현: WAL record/timestamp/hash manifest artifact 생성
...
| `orv db archive` | JSONL WAL record/timestamp/hash manifest 생성 |
...
| `orv db plan/verify/apply/migrate/rollback/backup/restore/recover/archive/squash` | schema diff dry-run/verify, snapshot apply/history/squash, 선택적 JSON data snapshot migrate/rollback, local backup/restore, WAL record recovery/archive manifest |
