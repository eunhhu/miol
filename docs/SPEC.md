...
> 구현 delta: 서버 build는 `server/runtime-image.json` native runtime image plan, `server/native/routes.rs` native route table source, `server/native/router.rs` native router dispatch source도 출력한다. image plan은 현재 reference image, planned OCI image tag, native binary path, route/listen/runtime feature shape, `native-codegen`/`native-runtime-image` blocker를 기록하며 `deploy/manifest.json`의 `server.native_runtime_image_plan`에서 참조된다. routes source는 server runtime artifact의 method/path/origin id 목록, contained `@respond` origin ids, `:param` 및 `:rest*` segment를 인식하고 route params를 `OrvNativeParam`으로 캡처하는 `orv_native_match_route` helper, `orv_native_param_value` lookup helper를 Rust source로 고정한다. router source는 routes source를 소비하고 matched dispatch에 route origin id, first response origin id, params를 싣는 `orv_native_dispatch` 501/404 dispatch contract를 Rust source로 고정하며, 두 source는 `server.native_routes_source`/`server.native_router_source`, native plan `routes_source`/`router_source`, reveal production payload에서 참조된다.
...
| `orv reveal <dir> <origin-id>` | build artifact origin id를 source span, ProjectGraph node, route descriptor, native server target/build-run command/response-origin dispatch 여부가 포함된 routes/router source summary/runtime image plan 또는 client manifest/bundle target으로 역추적 |
