...
> Native direct launcher delta: generated `server/native/main.rs` no longer shells through `orv run-artifact` for the native launcher path. It still validates the native plan and server artifact, then binds a dependency-free HTTP/1 listener using `ORV_HOST`, `ORV_PORT`, or the source-backed listen env/default port and dispatches requests through generated `routes.rs`/`router.rs`/`handlers.rs`. Static literal responses are served directly with origin headers; unsupported dynamic bodies keep the generated 501 placeholder.
