...
> Native trace frame action delta: `native-host.json` `trace.frames[*]` now exposes flattened `source`, `production`, and `reveal_command` fields in addition to the full navigation object, giving external native editor hosts a direct route from captured request frame to source/production reveal command.
