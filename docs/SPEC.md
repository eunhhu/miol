...
> DAP 세부 상태: 현재 external editor bootstrap은 source path/sourceReference 기반 verified nested conditional/hit-condition line, function/data breakpoint, goto/step-in target line, ORV exception filters, reference-runtime debug frame locals, captured function call-stack `stackTrace`, source/module inventory, local-name evaluate/completion, Locals snapshot `setVariable`/`setExpression`, live-mode 보존 restart, captured `stepIn.targetId`, live targetId 거부, runtime-frame next step-over/stepIn target selection/stepOut/stepBack/restartFrame/goto progression with termination, next line/function/data breakpoint continue/reverseContinue, per-frame stdout/stderr output events, stdio initialized/stopped/continued/terminated events, terminateThreads를 포함한다. `launch.arguments.live=true`는 launch 시 전체 프로그램을 선실행하지 않고 첫 frame까지만 실행한 뒤 `next`/`stepIn`/`stepOut`/`continue`에서 runtime frame을 증분 실행한다. 장기 실행 `@server` launch는 runtime을 직접 기동하지 않고 paused frame으로 열리며 `continue`/`pause` events와 async runtime state/counter inspect를 제공한다. 실제 async accept loop는 아직 없다.
