...
> Native direct launcher delta: generated `server/native/main.rs` validates the native plan and server artifact, then uses the generated route response metadata to choose an execution path. Route artifacts whose handlers are all static JSON, direct route-param JSON objects such as `{ id: @param.id }`, direct query-param JSON objects such as `{ q: @query.q }`, direct request-body JSON passthrough objects such as `{ received: @body }`, or direct request-body field JSON objects such as `{ handle: @body.handle }` bind a dependency-free HTTP/1 listener using `ORV_HOST`, `ORV_PORT`, or the source-backed listen env/default port and dispatch through generated `routes.rs`/`router.rs`/`handlers.rs`; artifacts with casts/DB or otherwise unsupported dynamic route bodies keep the reference `orv run-artifact` bridge so current fullservice behavior remains deployable until broader dynamic native lowering exists.
