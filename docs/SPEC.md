...
> DAP 세부 상태: breakpoint verification은 ProjectGraph selectable node와 AST statement line을 사용하고, conditional breakpoint는 Locals 값 조건으로 필터링하며, function breakpoint는 captured call-stack 이름과 매칭하고, data breakpoint는 Locals 변수 값 변경 frame과 매칭한다. Exception breakpoint filters는 diagnostics/runtime error 두 축을 제공한다. Locals scope는 reference runtime debug trace가 캡처한 현재 frame env 값을 노출하며, 함수 호출 결과와 재할당 이후 값도 `evaluate`와 `completions`에 반영된다. `setVariable`/`setExpression`은 live interpreter mutation 전 단계로 현재 stopped frame의 Locals snapshot 값을 갱신한다. `stackTrace`는 trace frame에 캡처된 function call stack을 노출한다. stdio event lifecycle은 initialize/configurationDone/continue/terminate/restart 계열 bootstrap event와 stdout/stderr output event를 emit한다. step 계열은 captured runtime frame progression으로 모델링하고 frame stdout delta를 output event로 emit하며, 마지막 frame 이후 세션을 종료한다. live long-running pause/resume loop는 아직 로드맵이다.
...
> DAP 세부 상태: 현재 external editor bootstrap은 verified conditional line/function/data breakpoint/goto target line, ORV exception filters, reference-runtime debug frame locals, captured function call-stack `stackTrace`, source/module inventory, local-name evaluate/completion, Locals snapshot `setVariable`/`setExpression`, restart, runtime-frame step/stepBack/goto progression with termination, next line/function/data breakpoint continue/reverseContinue, per-frame stdout/stderr output events, stdio initialized/stopped/continued/terminated events를 포함한다. 실제 live interpreter pause/resume loop는 아직 없다.
