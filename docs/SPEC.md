...
`orv.toml`은 프로젝트 설정 파일이다. Cargo.toml 스타일을 따른다. 현재 구현은 최소 `[project].entry`를 읽어 `run`/`check`/`origins`/`graph`/`editor snapshot`/`lsp snapshot`/`build`/`db plan`/`db apply` 같은 source-entry 명령의 entry 파일로 해석한다. `[dependencies]`와 `[dev-dependencies]`는 `orv add/remove`로 편집하고 `orv lock`에서 lockfile metadata로 고정한다. workspace, build profile, FFI allowlist와 실제 dependency resolution은 로드맵이다.
