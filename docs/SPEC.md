...
> Editor DAP native control delta: exported debug controls carry exact per-control `orv editor run-debug debug/session-runner.json --control <name>` runner commands, and executable breakpoint rows carry `--breakpoint <path>:<line> --control continue` commands plus `setBreakpoints` request previews. `native-host.json` mirrors launch/live/attach configurations under `debug.configurations`, controls under `debug.control_commands`, and executable breakpoint actions under `debug.breakpoint_commands`; the static shell renders the selected runner command next to the DAP request payload.
