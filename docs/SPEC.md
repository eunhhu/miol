...
> 구현 delta: 서버 build는 `server/runtime-image.json` native runtime image plan도 출력한다. 이 artifact는 현재 reference image, planned OCI image tag, native binary path, route/listen/runtime feature shape, `native-codegen`/`native-runtime-image` blocker를 기록하며 `deploy/manifest.json`의 `server.native_runtime_image_plan`에서 참조된다.
