...
> Dev HMR 상태: `orv dev --hmr`은 `dev/session.json`에 source hash watch set, bundle target, hot-reload/full-reload fallback 전략을 출력하고 `orv verify-build`는 이 session이 있으면 같은 source/bundle 계약으로 검증한다. 지속 watch loop와 브라우저 HMR transport는 아직 로드맵이다.
...
| `orv verify-build <dir>` | build manifest/plan target, source bundle hash, server runtime/launch artifact, static page zero-runtime shape, client page/JS/WASM metadata와 `orv_start` bootstrap, optional dev HMR session 검증 |
...
| `orv dev <file-or-orv.toml> --out <dir> [--hmr]` | build + verify-build + run-build reference dev bootstrap, `--hmr`이면 dev session manifest 출력 |
...
| `orv dev --watch` | 개발 서버 변경 감지 loop와 HMR |
| `orv add <pkg>` | 의존성 추가 |
