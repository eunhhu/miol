...
> DAP 세부 상태: 현재 external editor bootstrap은 verified breakpoint line, ORV exception filters, reference-runtime debug frame locals, captured function call-stack `stackTrace`, local-name evaluate/completion, restart, runtime-frame step progression with termination, next-breakpoint continue, per-frame stdout/stderr output events, stdio initialized/stopped/continued/terminated events를 포함한다. 실제 live interpreter pause/resume loop는 아직 없다.
