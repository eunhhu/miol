...
`orv.toml`은 프로젝트 설정 파일이다. Cargo.toml 스타일을 따른다. 현재 구현은 최소 `[project].entry`를 읽어 `run`/`check`/`origins`/`graph`/`editor snapshot`/`lsp snapshot`/`build`/`db plan`/`db apply` 같은 source-entry 명령의 entry 파일로 해석한다. `[dependencies]`와 `[dev-dependencies]`는 `orv add/remove`로 편집하고 `orv lock`에서 lockfile metadata로 고정한다. `[workspace].members`는 `orv workspace new/graph/build`의 member discovery와 workspace artifact 생성에 사용한다. 실제 dependency resolution, registry download, FFI allowlist는 로드맵이다.
...
`orv.lock`은 의존성의 정확한 버전을 기록하는 잠금 파일이다. 자동 생성되며, 버전 관리에 포함한다. 현재 `orv lock [dir-or-orv.toml]`은 `[project]`의 `name`/`version`/`entry`, registry/path dependency source, dependency version, stable `fnv1a64` checksum을 deterministic JSON으로 기록하고, `--check`는 기존 lockfile이 최신인지 비교한다.
