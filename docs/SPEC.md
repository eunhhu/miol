...
| `orv dev <file-or-orv.toml> --out <dir> [--hmr] [--watch] [--watch-loop]` | build + verify-build + run-build reference dev bootstrap, `--hmr`이면 dev session manifest, `--watch`이면 watch session manifest 출력, `--watch-loop`이면 poll loop event manifest 기록 |
