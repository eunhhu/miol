...
- `orv db archive --wal db.wal.jsonl --out archive.json [--target file:///archive]` — 현재 구현: WAL record/timestamp/hash manifest artifact 생성 및 file archive target 복사
- `orv db backup --target s3://bucket/path` — 로드맵: 포인트-인-타임 백업 (WAL 아카이브)
- `orv db restore --at "2026-04-17T12:00Z"` — 로드맵: 임의 시점 복구 (PITR)
- `@db.connect "postgres://..."` — 로드맵: 외부 DB 어댑터 (PostgreSQL/MySQL/SQLite). 기존 자산 이전/상호운용용이며, 프로덕션 기본 경로로는 비권장
...
> 구현 상태: 부분 구현. 현재 `orv-compiler`는 origin map, 초기 build manifest, bundle plan, build-level source bundle, source-bundled server runtime descriptor artifact, reference server launch artifact를 만들고, manifest에 필요한 `runtime_features`만 기록하지만, native production codegen/최적화 단계는 아직 없다. CLI는 HIR을 레퍼런스 런타임으로 직접 실행하고, `orv build <file-or-orv.toml> --out <dir>`은 manifest/bundle-plan/origin-map/project-graph/source-bundle/server-runtime/launch JSON을 출력한다. `orv build --prod`는 static `@listen 0`을 test-only ephemeral port로 거부하고, 같은 build directory에 `deploy/manifest.json`과, 서버가 있으면 `deploy/routes.json`, `deploy/container.json`, `deploy/Dockerfile`, `deploy/compose.yaml`, request trace capture/editor trace 명령이 포함된 `deploy/README.md`, `deploy/server.sh` reference server entrypoint를 추가한다. `source-bundle.json`은 source path/source/content hash를 담아 원본 파일 없이도 reveal/LSP reveal source span을 복구한다. Server 없는 HTML-only entry는 zero-runtime `pages/index.html` 정적 페이지도 출력하며, 해당 bundle target의 `runtime_features`는 빈 배열이다. `let sig` 또는 client-side HTML await가 필요한 entry는 zero-runtime static page 대신 non-zero-runtime `pages/index.html` shell, `ORV_CLIENT_BOOTSTRAP` metadata와 `orv_start` call을 담은 `client/app.js` bootstrap, `orv.client` custom section과 `orv_start` export를 담은 유효 WASM module인 `client/app.wasm`을 출력하고, bundle/deploy manifest에 `client_page`/`client_js`/`client_wasm` target을 기록한다. 생성된 server runtime artifact와 client bootstrap artifacts는 검증 가능하고, server runtime/launch artifact는 route descriptor와 listen origin/static/env port descriptor를 기록하며, `server/launch.json`, prod deploy entrypoint, reference container/Compose/runbook contract는 `orv run-artifact server/app.orv-runtime.json` 실행 계약을 기록한다. 이 장의 native binary/실제 WASM codegen 모델은 프로덕션 컴파일러가 갖춰진 뒤의 목표 모델이다.
...
> 구현 상태: 부분 구현. 현재 CLI는 `orv init <dir>` basic/shop 프로젝트 scaffold, shop `GET /` HTML form 홈 route와 shop 검증/배포 README, 단일 entry 파일 또는 `orv.toml` `[project].entry` 기준 `run`, `dev`, `check`, `dump`, `origins`, `graph`, `test`, `editor snapshot`, `editor reveal`, `editor runtime`, `editor export`, `editor trace`, `lsp snapshot`, `lsp reveal`, `lsp serve --stdio`, `dap serve --stdio`, `build`, `build --prod`, `add`, `remove`, `lock`, `fetch`, `workspace new`, `workspace graph`, `workspace lock`, `workspace fetch`, `workspace build`, `verify-build`, `verify-artifact`, `check-artifact`, `run-artifact`, `run-build`, `reveal`, `db plan/verify/apply/migrate/rollback/backup/restore/recover/archive/squash`를 제공한다. `@server` request body는 JSON과 `application/x-www-form-urlencoded`를 `@body` object로 노출한다. `orv-project`는 멀티파일 source map과 AST ProjectGraph v1을 생성하고, `orv graph <file>`은 이를 source/semantic graph depth stats, HIR origin map, HIR `contains`/`calls` origin edge, origin-to-AST-node link와 함께 JSON으로 출력하고, `--view --out <dir>`은 같은 데이터를 `graph.json`과 정적 `index.html` graph view로 쓴다. `orv test <path> --filter <name>`은 `.orv` 파일을 찾아 `test "name"` 블록이 있는 파일을 reference runtime 으로 실행하고 `--list`로 발견 목록 JSON을 출력한다. `orv editor snapshot <file>`은 같은 프로젝트 그래프와 diagnostics를 first-party editor bootstrap JSON으로 출력하고 Files/Routes/Schema/Domains 패널 입력 및 source-hash live refresh watch set을 제공한다. `orv editor reveal <dir> <origin-id>`는 build artifact origin을 first-party editor focus/source/production navigation payload로 변환한다. `orv editor runtime <file>`은 DAP trace/runtime helpers를 재사용해 runtime status/stdout/frame list를 first-party editor inspection pane JSON으로 출력한다. `orv editor export <file> --out <dir>`은 snapshot/runtime state JSON과 static HTML editor shell artifact를 출력하며 ProjectGraph view와 panel list를 렌더링하고, `--build <dir> --trace <trace.json>`이 있으면 trace navigation state도 embed한다. `orv editor trace <dir> --trace <trace.json>`은 captured request trace frame을 editor source/production navigation payload와 request summary/status bucket, trace-file live refresh contract, exported editor status filter로 확장한다. `orv dap serve --stdio`는 Debug Adapter Protocol  bootstrap을 제공하고 launch 시 프로젝트 그래프와 진단을 읽어 entry source stack frame, line/function/data breakpoint location list, goto/step-in target list, exception status, loaded source list/content, project module list, project-scope variables/evaluate/completion, Locals snapshot editing, reference runtime stdout/status/error variables를 반환한다. `orv build <file-or-orv.toml> --out <dir>`은 `build-manifest.json`, `bundle-plan.json`, `origin-map.json`, `project-graph.json`, `source-bundle.json`, source-bundled `server/app.orv-runtime.json`, `server/launch.json` artifact directory를 만들며, 순수 HTML-only entry는 zero-runtime `pages/index.html`, `let sig` 또는 client-side HTML await가 필요한 entry는 non-zero-runtime `pages/index.html` shell, `ORV_CLIENT_BOOTSTRAP` metadata와 `orv_start` call을 담은 JS bootstrap, `orv.client` custom section과 `orv_start` export를 담은 유효 WASM module인 `client/app.wasm`을 출력한다. Server runtime/launch artifact는 route descriptor와 listen origin/static/env port descriptor를 함께 기록한다. `orv build --prod`는 추가로 `deploy/manifest.json`, `deploy/routes.json`, `deploy/container.json`, `deploy/Dockerfile`, `deploy/compose.yaml`, `deploy/README.md`, reference server entrypoint를 출력하고 client bundle이 있으면 `deploy/manifest.json`에 source bundle and page/loader/wasm client target도 기록한다. manifest는 `runtime_features`로 필요한 runtime layer만 선언하고 bundle plan은 future bundler target을 선언한다. `source-bundle.json`은 원본 파일 없이도 `orv reveal`/`orv lsp reveal` source span을 복구한다. `orv add/remove`는 `orv.toml` dependency section을 편집하고 `orv.lock`을 재생성한다. `orv workspace new`는 workspace root manifest의 `[workspace].members`를 갱신하고 member project scaffold를 생성한다. `orv workspace graph`는 root `[workspace].members`를 읽고 각 member의 ProjectGraph/files/dependencies와 workspace member 간 path dependency edge를 JSON artifact로 출력한다. `orv workspace lock`은 같은 member graph를 입력으로 dependency-first 순서로 member lockfile artifact를 출력한다. `orv workspace fetch`는 같은 workspace lock artifact를 member dependency cache로 materialize한다. `orv workspace build`는 같은 member graph를 입력으로 path dependency edge 기반 dependency-first 순서로 각 member를 기존 build pipeline으로 빌드/검증하고 `members/<member>` 산출물, `workspace-graph.json`, `workspace-build.json`을 출력하며, `--incremental`은 input hash가 같은 verified member를 skip한다. `orv lock [dir-or-orv.toml]`은 `[project]`, `[dependencies]`, `[dev-dependencies]` metadata에서 deterministic JSON `orv.lock`을 생성하고, local/file/HTTP registry `index.json`에서 prerelease/build-aware exact/wildcard/caret/tilde/comparator/disjunction range를 exact locked version으로 고정하며, optional `auth_token_env` HTTP Bearer auth env var 이름을 secret 없이 보존하고 `--check`로 최신성을 검증한다. `orv verify-build <dir>`은 manifest/plan target path, source bundle hash, deploy manifest/entrypoint/routes inventory/container/Dockerfile/Compose/runbook, server runtime/launch artifact와 listen 일치성, static page zero-runtime/HTML shape, client page/JS bootstrap `orv_start` call, parsed `orv.client` WASM metadata/export, optional dev HMR/watch/transport/event/server manifests를 검증한다. `orv verify-artifact <file>`은 source hash와 route descriptor shape를 검증하고, `orv check-artifact <file>`은 import 포함 server artifact source bundle을 재분석하며, `orv check-build <dir>`은 build-level source bundle을 재분석하고, `orv run-artifact <file>`은 검증된 source bundle을 재수화해 reference runtime으로 실행한다. `orv run-build <dir>`은 bundle plan target을 기준으로 연결된 reference artifact를 실행하거나, server 없는 static/client page build에서는 verified HTML을 출력한다. `orv dev <file-or-orv.toml> --out <dir>`은 build, verify-build, run-build를 순서대로 수행하는 reference dev bootstrap이고, `--hmr --serve`는 reference HTTP/1 EventSource endpoint를 제공한다. `orv reveal <dir> <origin-id>`는 build artifact origin id를 source span, ProjectGraph node, server route descriptor 또는 client bundle target으로 역추적한다. 전체 HTTPS/TLS registry transport와 native production server/실제 WASM 번들은 로드맵이다.
...
`orv.toml`은 프로젝트 설정 파일이다. Cargo.toml 스타일을 따른다. 현재 구현은 최소 `[project].entry`를 읽어 `run`/`check`/`origins`/`graph`/`editor snapshot`/`lsp snapshot`/`build`/`db plan`/`db apply` 같은 source-entry 명령의 entry 파일로 해석한다. `[dependencies]`와 `[dev-dependencies]`는 `orv add/remove`로 편집하고 `orv lock`에서 lockfile metadata로 고정하며, local/file/HTTP registry `index.json`이 있으면 prerelease/build-aware exact/wildcard/caret/tilde/comparator/disjunction range를 exact locked version으로 해석한다. Registry dependency table의 `auth_token_env`는 HTTP registry index/fetch 요청에 사용할 Bearer token env var 이름이다. `orv fetch`는 path/local registry dependency와 HTTP registry source-bundle endpoint를 source-bundle cache로 materialize한다. `[workspace].members`는 `orv workspace new/graph/build`의 member discovery와 workspace artifact 생성에 사용한다. HTTPS/TLS registry transport, FFI allowlist는 로드맵이다.
...
`orv.lock`은 의존성의 정확한 버전을 기록하는 잠금 파일이다. 자동 생성되며, 버전 관리에 포함한다. 현재 `orv lock [dir-or-orv.toml]`은 `[project]`의 `name`/`version`/`entry`, registry/path dependency source, optional `auth_token_env`, prerelease/build-aware exact dependency version, local/file/HTTP registry `index.json` 기반 wildcard/caret/tilde/comparator/disjunction range resolution 결과, 필요 시 원래 range `requested_version`, stable `fnv1a64` checksum을 deterministic JSON으로 기록하고, `--check`는 기존 lockfile이 최신인지 비교한다. `orv fetch`는 최신 lockfile만 받아 path dependency, relative/`file://` local registry dependency, HTTP registry `/<package>/<version>/source-bundle.json` endpoint를 `deps-manifest.json` 및 `packages/<section>/<name>/source-bundle.json`으로 가져온다.
...
| `orv build <file-or-orv.toml> --out <dir> [--prod]` | 초기 build manifest + bundle plan + origin map + project graph + source bundle + server runtime/launch artifact, HTML-only static page, 또는 client page/JS/WASM bootstrap 출력, prod profile이면 deploy manifest/container/Dockerfile/Compose/runbook/entrypoint 추가 |
...
| `orv lock [dir-or-orv.toml] [--check]` | `orv.toml` project/dependency metadata와 local/file/HTTP registry `index.json` 기반 prerelease/build-aware exact/wildcard/caret/tilde/comparator/disjunction range 결과에서 deterministic `orv.lock` 생성 또는 최신성 검증 |
| `orv fetch [dir-or-orv.toml] --out <dir>` | 최신 `orv.lock` 기준 path/local registry 및 HTTP registry dependency source-bundle cache와 `deps-manifest.json` 출력 |
| `orv workspace new <member> [--root <dir>] [--name <name>]` | workspace root manifest member 등록과 member project scaffold 생성 |
| `orv workspace graph [root] [--out <dir>]` | workspace member ProjectGraph/files/dependencies 및 member 간 path dependency edge artifact 출력 |
...
| `orv verify-build <dir>` | build manifest/plan target, source bundle hash, deploy route inventory/container/runtime_image/ports/Dockerfile/Compose/runbook, server runtime/launch artifact, static page zero-runtime shape, client page/JS/WASM metadata와 `orv_start` bootstrap, optional dev HMR/watch/transport/event/server manifest 검증 |
...
| `orv reveal <dir> <origin-id>` | build artifact origin id를 source span, ProjectGraph node, route descriptor 또는 client bundle target으로 역추적 |
| `orv test <path> --filter <name> --list` | `.orv` test block 파일 탐색 + reference runtime 실행 또는 발견 목록 JSON 출력 |
| `orv editor snapshot/reveal/runtime/export/trace` | first-party editor bootstrap JSON + build-origin navigation payload + runtime inspection pane JSON + static editor shell artifact with ProjectGraph/panel-list/runtime-frame-detail/trace-detail rendering and optional trace state + captured request trace navigation payload 출력 |
...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈 및 회원/결제/배송 route scaffold와 검증/Compose 배포 README 생성 |
...
| `orv db restore --at` | timestamp 기반 임의 시점 DB 복구 |
