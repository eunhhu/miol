...
- `@db.connect "memory://local"` — 현재 구현: reference in-memory handle 반환. `postgres://`/`mysql://`/`sqlite://` 외부 DB 어댑터는 로드맵이며 현재는 명시적으로 실패한다. 기존 자산 이전/상호운용용이며, 프로덕션 기본 경로로는 비권장
