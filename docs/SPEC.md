...
> DAP 세부 상태: `setBreakpoints`는 ProjectGraph selectable node와 AST statement 시작 라인을 합쳐 빈 줄/비실행 라인을 unverified 처리한다. `setExceptionBreakpoints`는 `orv.diagnostics`와 `orv.runtime` filters를 수용한다. `scopes`/`variables`는 project metadata 외에 stopped line 이하의 top-level `let`/`const` Locals scope를 제공하고, pure arithmetic/string/bool/array/object expression initializer와 이전 local 참조를 값으로 표시한다. `evaluate`/`completions`도 현재 stopped line에서 보이는 local 이름을 인식한다. `next`/`stepIn`/`stepOut`은 entry 파일의 다음 executable line으로 이동하고 마지막 executable line 이후에는 terminated event로 세션을 닫는다. `pause`는 현재 line에서 `stopped` event를 내고, `restart`는 현재 program을 재로딩한다. stdio 서버는 stdout/stderr `output` event와 `initialized`, `stopped`, `continued`, `terminated` event frame을 response 뒤에 emit한다.
...
> DAP 세부 상태: breakpoint verification은 ProjectGraph selectable node와 AST statement line을 사용한다. Exception breakpoint filters는 diagnostics/runtime error 두 축을 제공한다. Locals scope는 stopped line 이하의 top-level `let`/`const` pure expression/compound literal 값을 노출하며, local 이름은 `evaluate`와 `completions`에서도 동작한다. stdio event lifecycle은 initialize/configurationDone/continue/terminate/restart 계열 bootstrap event와 stdout/stderr output event를 emit한다. step 계열은 entry executable line progression으로 모델링하고 마지막 line 이후 세션을 종료한다. 런타임 실행 중 변하는 expression/local capture는 아직 로드맵이다.
...
> DAP 세부 상태: 현재 external editor bootstrap은 verified breakpoint line, ORV exception filters, stopped-line-filtered top-level pure-expression/compound Locals scope, local-name evaluate/completion, restart, entry-line step progression with termination, stdout/stderr output events, stdio initialized/stopped/continued/terminated events를 포함한다. 실제 interpreter pause/resume loop와 동적 expression frame locals는 아직 없다.
