...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, `GET /catalog` customer catalog, `GET /admin` 운영 dashboard, catalog/order/payment/shipment/webhook admin read models, 회원/결제/배송/Stripe webhook route scaffold, env-overridable `SHOP_DATABASE_URL` SQLite default, env-overridable local `@payment`/`@shipping` reference adapter record log, 검증/Compose 배포 README 생성 |
