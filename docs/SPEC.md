...
- `orv db recover --wal db.wal.jsonl --out data.json --until-record 128` 또는 `--until-unix-ms 1770000000000` — 현재 구현: JSONL WAL을 record count 또는 unix ms timestamp 경계까지 재생해 `@db.save` 호환 snapshot 복구
...
| `orv db recover` | JSONL WAL을 record count 또는 unix ms timestamp 경계까지 재생해 data snapshot 복구 |
...
| `orv dap serve --stdio` | DAP  bootstrap |
