...
> Provider webhook 상태: Stripe provider payment handles expose `payments.verifyWebhook({ payload, signature })` in the reference runtime. It validates Stripe-style HMAC-SHA256 signatures from `STRIPE_WEBHOOK_SECRET` and returns status metadata without exposing secrets or payloads. The shop scaffold hosts `POST /webhooks/stripe` as a reference endpoint that passes raw request body and `stripe-signature` through that verifier and stores `WebhookEvent` rows; idempotency and provider reconciliation remain roadmap work.
