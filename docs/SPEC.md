...
- `orv db recover --wal db.wal.jsonl --out data.json --until-record 128` — 현재 구현: JSONL WAL을 처음 N개 complete record까지 재생해 `@db.save` 호환 snapshot 복구
...
| `orv db recover` | JSONL WAL을 record 경계까지 재생해 data snapshot 복구 |
