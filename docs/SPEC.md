...
`@server` boot body에서 실행한 `@db.wal(path)`/`@db.load(path)` 설정 또는 `let shopdb = @db.connect "sqlite://data/shop.sqlite"` 같은 adapter handle은 route handler에 전달된다. 따라서 shop scaffold는 SQLite-backed `shopdb` handle을 route에서 캡처해 서버 시작 시 row JSON을 reload하고 이후 상품/회원/주문/결제/배송 mutation을 같은 SQLite 파일에 기록한다.
...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, `GET /admin` 운영 dashboard, 회원/결제/배송 route scaffold, SQLite-backed `data/shop.sqlite` DB adapter, file-backed local `@payment`/`@shipping` reference adapter record log, 검증/Compose 배포 README 생성 |
