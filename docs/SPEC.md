...
`orv.toml`은 프로젝트 설정 파일이다. Cargo.toml 스타일을 따른다. 현재 구현은 최소 `[project].entry`를 읽어 `run`/`check`/`origins`/`graph`/`editor snapshot`/`lsp snapshot`/`build`/`db plan`/`db apply` 같은 source-entry 명령의 entry 파일로 해석한다. `[dependencies]`와 `[dev-dependencies]`는 `orv add/remove`로 편집하고 `orv lock`에서 lockfile metadata로 고정하며, `orv fetch`는 path/local registry dependency와 HTTP registry source-bundle endpoint를 source-bundle cache로 materialize한다. `[workspace].members`는 `orv workspace new/graph/build`의 member discovery와 workspace artifact 생성에 사용한다. HTTPS/authenticated registry transport, full semver registry resolution, FFI allowlist는 로드맵이다.
...
| `orv lock [dir-or-orv.toml] [--check]` | `orv.toml` project/dependency metadata에서 deterministic `orv.lock` 생성 또는 최신성 검증 |
