...
`orv.toml`은 프로젝트 설정 파일이다. Cargo.toml 스타일을 따른다. 현재 구현은 최소 `[project].entry`를 읽어 `run`/`check`/`origins`/`graph`/`editor snapshot`/`lsp snapshot`/`build`/`db plan`/`db apply` 같은 source-entry 명령의 entry 파일로 해석한다. 의존성, workspace, build profile, FFI allowlist는 로드맵이다.
...
| `orv verify-build <dir>` | build manifest/plan target, source bundle hash, deploy route inventory/container/runtime_image/ports/Dockerfile/Compose/runbook, server runtime/launch artifact, static page zero-runtime shape, client page/JS/WASM metadata와 `orv_start` bootstrap, optional dev HMR/watch session 검증 |
...
| `orv dev <file-or-orv.toml> --out <dir> [--hmr] [--watch]` | build + verify-build + run-build reference dev bootstrap, `--hmr`이면 dev session manifest, `--watch`이면 watch session manifest 출력 |
...
| `orv editor snapshot/reveal/runtime/export/trace` | first-party editor bootstrap JSON + build-origin navigation payload + runtime inspection pane JSON + static editor shell artifact with panel-list/trace-detail rendering and optional trace state + captured request trace navigation payload 출력 |
| `orv lsp snapshot/reveal/serve --stdio` | 에디터 bootstrap JSON, production reveal, LSP initialize/diagnostic/symbol/code-lens/code-action/folding/selection/semantic-tokens/definition/declaration/typeDefinition/implementation/call-hierarchy/type-hierarchy/references/highlight/rename/hover/completion 처리 |
...
| 지속 HMR dev server | 개발 서버 변경 감지 process loop와 browser/server transport |
| `orv add <pkg>` | 의존성 추가 |
