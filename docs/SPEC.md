...
> Native direct launcher delta: generated `server/native/main.rs` validates the native plan and server artifact, then uses the generated route response metadata to choose an execution path. All-static route artifacts bind a dependency-free HTTP/1 listener using `ORV_HOST`, `ORV_PORT`, or the source-backed listen env/default port and dispatch through generated `routes.rs`/`router.rs`/`handlers.rs`; artifacts with dynamic or unsupported route bodies keep the reference `orv run-artifact` bridge so current fullservice behavior remains deployable until dynamic native lowering exists.
