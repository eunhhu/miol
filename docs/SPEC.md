...
> Provider webhook 상태: Stripe provider payment handles expose `payments.verifyWebhook({ payload, signature })` in the reference runtime. It validates Stripe-style HMAC-SHA256 signatures from `STRIPE_WEBHOOK_SECRET` and returns status metadata without exposing secrets or payloads. The shop scaffold hosts `POST /webhooks/stripe` as a reference endpoint that passes raw request body and `stripe-signature` through that verifier, stores `WebhookEvent.eventId`, and returns the existing row for duplicate provider/eventId deliveries; provider reconciliation remains roadmap work.
...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, `GET /admin` 운영 dashboard, catalog/order/payment/shipment/webhook admin read models, 회원/결제/배송/Stripe webhook route scaffold, env-overridable `SHOP_DATABASE_URL` SQLite default, env-overridable local `@payment`/`@shipping` reference adapter record log, 검증/Compose 배포 README 생성 |
...
> Editor DAP native control delta: exported debug controls carry exact per-control `orv editor run-debug debug/session-runner.json --control <name>` runner commands, and executable breakpoint rows carry `--breakpoint <path>:<line> --control continue` commands plus `setBreakpoints` request previews. `native-host.json` mirrors launch/live/attach configurations under `debug.configurations`, controls under `debug.control_commands`, executable breakpoint actions under `debug.breakpoint_commands`, and the `debug/session-result.json` artifact under `debug.result_artifact`; the static shell renders the selected runner command, result artifact metadata, and DAP request payload, while runner results include `panels.debug` selected-frame/stack/control/breakpoint/event summaries for native UI refresh.
