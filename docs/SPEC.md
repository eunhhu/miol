...
> Provider webhook 상태: Stripe provider payment handles expose `payments.verifyWebhook({ payload, signature })` in the reference runtime. It validates Stripe-style HMAC-SHA256 signatures from `STRIPE_WEBHOOK_SECRET` and returns status metadata without exposing secrets or payloads; hosted provider webhook endpoint processing remains roadmap work.
...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, `GET /admin` 운영 dashboard, `GET /admin/catalog` catalog read model, 회원/결제/배송 route scaffold, env-overridable `SHOP_DATABASE_URL` SQLite default, env-overridable local `@payment`/`@shipping` reference adapter record log, 검증/Compose 배포 README 생성 |
