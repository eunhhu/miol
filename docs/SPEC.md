...
> Native direct launcher delta: generated `server/native/main.rs` validates the native plan and server artifact, then uses the generated route response metadata to choose an execution path. Route artifacts whose handlers are all static JSON, direct route-param JSON objects such as `{ id: @param.id }`, direct query-param JSON objects such as `{ q: @query.q }`, route/query numeric-cast JSON objects such as `{ id: @param.id as int }` or `{ page: @query.page as float }`, route/query int literal/captured arithmetic objects such as `{ next: (@param.id as int) + 1 }`, `{ parity: (@query.page as int) % 2 }`, `{ shifted: (@param.id as int) + (@param.offset as int) }`, or `{ next: (@query.page as int) + (@query.step as int) }`, direct request-body JSON passthrough objects such as `{ received: @body }`, direct request-body field JSON objects such as `{ handle: @body.handle }`, request-body field numeric-cast JSON objects, supported request-body int arithmetic JSON objects, mixed static/domain/arithmetic JSON objects such as `{ kind: "calc", next_id: (@param.id as int) + 1 }`, or simple request-body/route-param/query-param/cross-source guard flows, including integer comparison guards such as `if (@body.quantity as int) > 0 { @respond ... }` and captured integer comparisons such as `if (@body.quantity as int) <= (@body.stock as int) { @respond ... }`, bind a dependency-free HTTP/1 listener using `ORV_HOST`, `ORV_PORT`, or the source-backed listen env/default port and dispatch through generated `routes.rs`/`router.rs`/`handlers.rs`; artifacts with DB or otherwise unsupported dynamic route bodies keep the reference `orv run-artifact` bridge so current fullservice behavior remains deployable until broader dynamic native lowering exists.
