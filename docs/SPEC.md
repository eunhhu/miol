...
2026-05-06 보충: HTTP archive target upload/restore는 구현되어 `orv db archive --target http://host/archive`가 WAL과 manifest를 HTTP `POST`로 업로드하고 local manifest에 remote WAL/manifest URL을 기록한다. `orv db archive --target s3://bucket/prefix`는 `ORV_DB_ARCHIVE_S3_ENDPOINT=http://host`를 S3-compatible endpoint로 사용해 WAL과 manifest를 HTTP `PUT`으로 업로드한다. source WAL이 없으면 `orv db restore/recover --archive`가 remote WAL을 HTTP `GET`으로 내려받아 hash/byte 검증 후 복원한다. signed S3/cloud object-store provider integration은 여전히 로드맵이다.
