...
> Client manifest 상태: interactive client entry는 `client/manifest.json`과 `client/reactive-plan.json`도 출력한다. manifest는 page shell, reactive plan, JS loader, WASM module path/hash, `source-bundle.json` stable hash, WASM ABI exports, initial-render metadata, runtime feature, dynamic/reactive codegen blocker를 하나로 묶고, build manifest/bundle plan/prod deploy manifest/reveal production payload에서 `client_manifest` 계약으로 추적된다. reactive plan은 source-backed `let sig` origin, checked initial-render binding from `client/app.wasm` to `pages/index.html`, source hash, future DOM diff blocker를 고정한다. 생성된 `client/app.js`는 manifest와 reactive plan을 먼저 fetch해 schema/source hash/WASM path/hash/export/reactive-plan/binding 계약을 검증한 뒤 source bundle과 WASM을 로드하고, WASM에서 decode한 initial render HTML의 hash와 byte length를 manifest의 `initial_render`와 대조한다. `orv verify-build`는 manifest/reactive plan이 생성 page/loader/WASM/source bundle과 같은 client bundle을 가리키고 loader가 manifest/reactive-plan/source-bundle/WASM-hash/initial-render 계약을 소비하는지 검증한다.
