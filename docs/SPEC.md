...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, 회원/결제/배송 route scaffold, SQLite-backed `data/shop.sqlite` DB adapter, file-backed local `@payment`/`@shipping` reference adapter record log, 검증/Compose 배포 README 생성 |
