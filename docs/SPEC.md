...
`orv.lock`은 의존성의 정확한 버전을 기록하는 잠금 파일이다. 자동 생성되며, 버전 관리에 포함한다. 현재 `orv lock [dir-or-orv.toml]`은 `[project]`의 `name`/`version`/`entry`, registry/path dependency source, dependency version, stable `fnv1a64` checksum을 deterministic JSON으로 기록하고, `--check`는 기존 lockfile이 최신인지 비교한다. `orv fetch`는 최신 lockfile만 받아 path dependency와 relative/`file://` local registry dependency를 `deps-manifest.json` 및 `packages/<section>/<name>/source-bundle.json`으로 가져온다.
...
| `orv fetch [dir-or-orv.toml] --out <dir>` | 최신 `orv.lock` 기준 path/local registry dependency source-bundle cache와 `deps-manifest.json` 출력 |
...
| `orv workspace build [root] --out <dir> [--prod]` | path dependency edge 기반 dependency-first 순서로 workspace member별 build/verify 실행 후 `members/<member>` artifacts, `workspace-graph.json`, `workspace-build.json` 출력 |
