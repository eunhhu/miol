...
| `orv dap serve --stdio` | DAP initialize/launch/configurationDone/setBreakpoints/breakpointLocations/exceptionInfo/threads/stackTrace/scopes/variables/setVariable/evaluate/setExpression/completions/loadedSources/source/continue/step/disconnect/terminate bootstrap |
...
> 구현 상태: 부분 구현. 현재 CLI는 `orv lsp snapshot`, `orv lsp reveal`, `orv lsp serve --stdio`의 initialize/shutdown stdio bootstrap과 `textDocument/documentSymbol`, `textDocument/codeLens`, `textDocument/codeAction`, `textDocument/documentLink`, `textDocument/foldingRange`, `textDocument/selectionRange`, `textDocument/semanticTokens/full`, `textDocument/diagnostic`, `workspace/diagnostic`, `workspace/executeCommand`, `textDocument/definition`, `textDocument/references`, `textDocument/documentHighlight`, `textDocument/prepareRename`, `textDocument/rename`, `textDocument/hover`, `textDocument/completion`, `workspace/symbol` file URI/root URI 응답, `textDocument/didOpen`/`textDocument/didChange` full-sync open-buffer cache를 제공한다. `orv dap serve --stdio`는 DAP initialize/launch/configurationDone/setBreakpoints/breakpointLocations/exceptionInfo/threads/stackTrace/scopes/variables/setVariable/evaluate/setExpression/completions/loadedSources/source/continue/step/disconnect/terminate bootstrap과 entry source stack frame/loaded source content/project variables/evaluate/reference runtime stdout/status/error variables/runtime debug frame locals/captured function call stack을 제공한다. Tree-sitter 패키지, 리팩터링 같은 완전한 LSP method set과 live pause/resume DAP loop는 아직 로드맵이다.

> DAP 세부 상태: 현재 external editor bootstrap은 verified breakpoint line, ORV exception filters, reference-runtime debug frame locals, captured function call-stack `stackTrace`, local-name evaluate/completion, Locals snapshot `setVariable`/`setExpression`, restart, runtime-frame step progression with termination, next-breakpoint continue, per-frame stdout/stderr output events, stdio initialized/stopped/continued/terminated events를 포함한다. 실제 live interpreter pause/resume loop는 아직 없다.
