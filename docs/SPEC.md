...
`@server` boot body에서 실행한 `@db.wal(path)`/`@db.load(path)` 설정 또는 `let shopdb = @db.connect(@env.SHOP_DATABASE_URL ?? "sqlite://data/shop.sqlite")` 같은 adapter handle은 route handler에 전달된다. 따라서 shop scaffold는 env-overridable SQLite-backed `shopdb` handle을 route에서 캡처해 서버 시작 시 row JSON을 reload하고 이후 상품/회원 가입/로그인 세션/주문/결제/배송 mutation을 같은 SQLite 파일에 기록한다. `POST /checkout`은 회원 조회, 재고 차감, 주문 생성, 결제 capture, 배송 booking, 주문 상태 갱신을 한 route에서 실행하고, `GET /catalog`는 persisted Product rows를 customer-facing browser catalog로 렌더링하며, `GET /account/sessions`는 active Session rows를 browser account view로 렌더링하고, `GET /admin/catalog`, `GET /admin/orders`, `GET /admin/payments`, `GET /admin/shipments`, `GET /admin/webhooks`는 persisted row를 browser read model로 렌더링하며, `GET /admin/summary`는 같은 handle의 `count` 집계를 사용해 상품/회원/주문/결제/배송/webhook row count를 운영 API로 노출한다.
...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, `GET /catalog` customer catalog, `GET /account/sessions` account session view, `GET /admin` 운영 dashboard, catalog/order/payment/shipment/webhook admin read models, 회원/결제/배송/Stripe webhook route scaffold, env-overridable `SHOP_DATABASE_URL` SQLite default, env-overridable local `@payment`/`@shipping` reference adapter record log, 검증/Compose 배포 README 생성 |
