...
> Editor DAP native control delta: exported debug controls carry exact per-control `orv editor run-debug debug/session-runner.json --control <name>` runner commands. `native-host.json` mirrors them under `debug.control_commands`, and the static shell renders the selected runner command next to the DAP request payload.
