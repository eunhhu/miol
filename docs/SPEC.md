...
2. **MVP/구현 계약 문서 — `docs/MVP.md`, `docs/IMPLEMENTATION_MATRIX.md`**
   - 지금 되는 것, MVP non-goal, 구현 상태, 계약 레벨, 담당 crate, fixture/CLI 검증 기준을 표로 추적한다.
3. **기준 사양 — `docs/SPEC.md`**
   - 현재 기준의 공식 문법, 의미론, 컴파일타임 규칙을 정의한다.
   - 날짜형 구현 델타나 운영 surface 목록을 포함하지 않는다.
4. **실험 사양 / 탐색 예제 — `fixtures/default-syntax.orv`, `fixtures/plan/*.orv`**
   - 사용감 검토, 문법 압박 테스트, 미래 방향 탐색을 위한 예제다.
5. **구현 아키텍처 — `docs/ARCHITECTURE.md`**
   - 현재 컴파일러/크레이트 구조와 데이터 흐름을 설명한다.

문서 간 충돌이 있으면 언어 의미론은 **`SPEC.md`**, 구현/계약 상태는 **`IMPLEMENTATION_MATRIX.md`**, 운영 surface는 **`OPERATIONAL_SURFACES.md`**를 우선 기준으로 해석한다. `fixtures/plan`과 `default-syntax.orv`는 중요한 설계 의도를 드러낼 수 있지만, 문법이나 의미론이 `SPEC.md`와 다르면 탐색 중 예제로 간주한다. `ARCHITECTURE.md`는 구현 구조의 기준 문서이며, 언어 의미론을 덮어쓰지 않는다.

예제를 읽을 때는 다음 순서를 권장한다.

- 방향성과 목표 이해: `docs/README.md`
- 현재 범위와 계약 레벨 확인: `docs/MVP.md`, `docs/IMPLEMENTATION_MATRIX.md`
...
SPEC은 언어 의미론의 기준 문서지만, 구현 중인 프로젝트에서 모든 절이 같은 안정 계약을 뜻하지는 않는다. 구현이 딸린 기능은 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)에서 다음 축으로 판정한다.

```md
Status: implemented | reference stub | artifact only | planned | not started
Contract: stable | stable-ish | experimental | reference | unstable | non-binding
Milestone: M0 | M1 | M2 | M3 | M4+
Crate: orv-*
Test: fixture/test path or planned
CLI: command surface
```

중요한 구분은 다음과 같다.

- `Status: implemented`는 코드 경로가 있다는 뜻이다. 외부 안정 계약이라는 뜻은 아니다.
- `Contract: stable` 또는 `stable-ish`만 사용자 문서와 scaffold가 강하게 의존할 수 있다.
- `Contract: experimental`은 구현이 있어도 문법/JSON shape/edge case가 바뀔 수 있다.
- `Contract: reference`는 reference runtime, local adapter, scaffold 기준 계약이다. production provider 품질을 뜻하지 않는다.
- `Contract: non-binding`은 로드맵/방향이다. SPEC에 목표 의미론으로 있어도 구현 의무나 호환성 약속이 아니다.

SPEC 본문이 목표 동작을 설명하고 matrix가 `planned` 또는 `non-binding`으로 표시한 경우, 구현팀은 matrix의 계약 레벨을 우선해 milestone을 잡는다.
...
| Surface | 현재 계약 의도 | Matrix 기준 |
|---------|----------------|-------------|
| Source span / AST / HIR origin | 같은 source 위치와 origin id가 parser, analyzer, compiler, runtime, reveal에서 유지되어야 함 | M0/M3 |
| `orv graph` / `orv origins` | 구조 그래프와 의미 origin edge를 같은 artifact 계열로 노출 | M0/M3 |
...
이 절은 Systems Surface 기준이다. App Authoring Surface의 쇼핑몰 작성자는 일반적으로 이 모델을 직접 다루지 않으며, scaffold와 표준 도메인은 필요한 소유권/복사/이동을 숨긴다.
...
`@body: T`, `@query: T`, `@form: T`는 요청 값을 `T.parse(...)`로 파싱하고 검증하는 선언형 바인딩이다. 검증 실패는 route의 표준 validation response로 변환되며, route handler 본문은 검증된 타입만 다룬다.
...
struct CheckoutForm {
  ...
  sku: string(nonempty)
  quantity: int(1..=99)
  shippingAddress: string(trim, min=10)
}

@route POST /checkout {
  @body: CheckoutForm
  let total = await priceFor(@body.sku, @body.quantity)
  @respond 200 { ok: true, total: total }
}
```

실패 응답의 기본 shape는 다음과 같다.
...
`try` 없이 발생한 `throw`는 가장 가까운 실행 경계까지 전파된다. 실행 경계마다 기본 정책이 다르며, 서버 route 에러는 서버 프로세스를 종료하지 않는다.

| 경계 | 기본 정책 |
|------|-----------|
| 일반 함수 | caller로 전파 |
| `@route` | active transaction rollback, safe 500 response, error audit/trace 기록, 서버 계속 실행 |
| `@job` / `@cron` | 실패 상태 기록 후 retry/backoff 정책 적용 |
| `spawn` | task failure로 기록, join/evaluate 시 Error 노출 |
| CLI/main | 진단 출력 후 non-zero exit |
| `@unsafe` | 안전 경계 밖으로 전파되면 compile warning; 명시적 catch 권장 |
...
@route GET /age {
  let age = mustBePositive(int.from(@query.age))
  @respond 200 { age: age }
}
// query가 잘못되면 기본 500 response 또는 route-level @catch 정책으로 변환된다.
```
...
**컴파일타임 보장:** 컴파일러는 각 함수의 "에러 시그니처"를 추론한다. `@route` 최상단, `spawn { }` 본문, `@job` 핸들러, `main` 본문 중 어느 곳까지 에러가 전파될 수 있는지 그래프로 파악한다. 경계의 기본 정책이 안전하지 않거나 모호하면 warning을 낸다.
...
FFI 구현 상태는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)를 따른다. App Authoring Surface에서는 FFI를 기본 문서/스캐폴드 경로에 노출하지 않는다.
...
아래 문법은 도메인 호출의 property/token/content 경계를 고정한다.

```ebnf
DomainDef       = ["pub"], "define", Ident, ParamList?, "->", DomainBody ;
ParamList       = "(", [Param, { Param }], ")" ;
Param           = Ident, ":", Type ;
DomainBody      = Expr | Block ;

DomainCall      = "@", DomainPath, { DomainArg }, [ContentBlock] ;
DomainPath      = Ident, { ".", Ident } ;
DomainArg       = PropertyArg | TokenArg ;
PropertyArg     = Ident, "=", Expr ;
TokenArg        = Literal | Ident | Path | Expr ;
ContentBlock    = "{", { Stmt }, "}" ;
```

`=`가 있는 인자는 항상 property다. `=`가 없는 인자는 항상 token이다. `{}` content block은 domain call의 마지막 요소여야 한다.

### 9.0.2 Lowering

도메인 호출은 분석 단계에서 다음 형태로 정규화된다.

```orv
@MyDomain prop1=5 23px "label" {
  @out "body"
}
```
...
1. property는 token 후보가 아니다.
2. token declaration 순서대로 첫 번째로 매칭되는 타입에 들어간다.
3. 한 token이 어떤 선언에도 매칭되지 않으면 해당 token span에 type error를 붙인다.
4. 문자열 리터럴 token과 identifier token은 lexical form을 유지한다. identifier token이 현재 lexical scope의 값으로 해석되는 경우에는 그 값의 타입을 기준으로 매칭한다.
5. 동일 token이 둘 이상에 매칭될 수 있으면 먼저 선언된 token field가 이긴다. 모호성을 피하려면 더 구체적인 패턴을 먼저 둔다.
...
`@content`가 없는 도메인에 `{}` 블록을 전달하면 compile warning을 낸다. `@content`는 가장 가까운 domain call의 content slot만 참조한다. content 블록은 caller의 lexical scope에서 이름을 해석한 뒤 callee의 `@content` 위치에 삽입되어 실행된다. 따라서 실행 순서는 callee body 안의 `@content` 위치가 결정하고, scope는 caller가 결정한다.

### 9.6 중첩 도메인 (Parent.Child)

도메인 내부에서 `define`으로 하위 도메인을 선언할 수 있다. 하위 `define` 선언은 부모 domain의 lexical member로 hoist된다. Hoist는 이름 해석만을 위한 것이며, 실행 순서를 바꾸지 않는다. 외부에서는 `Parent.Child` 경로로 접근하고, 해당 도메인 스코프 내에서는 `Child`만으로 접근 가능하다. 깊은 중첩도 가능하다 (`@App.Nav.Item`).
...
전송 프로토콜 구현 상태는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)를 따른다. 이 절은 route의 언어 의미론만 정의한다.
...
Typed binding은 §4.10의 schema validator를 사용한다.
...
`@body: T`가 실패하면 route body는 실행되지 않고 표준 validation response가 반환된다.
...
App Authoring Surface에서는 직접 bearer token을 자르는 대신 안전한 선언형 도메인을 우선 사용한다.
...
`@db`는 schema, query, transaction, migration을 하나의 도메인 표면으로 제공한다. 제품 초기의 production path는 SQLite/PostgreSQL 같은 검증된 adapter와 schema/migration DSL을 우선한다. 자체 orv-db storage engine과 optimizer는 장기 차별화 로드맵이다.
...
**운영 세부:** `@db` adapter 세부, shop scaffold persistence, webhook, archive/PITR 운영 델타는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md), [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md), [CHANGELOG.md](CHANGELOG.md)를 따른다. 이 절은 DB 도메인의 목표 의미론만 정의한다.

**MVP persistence contract:** 초기 production path는 SQLite/PostgreSQL adapter, schema snapshot, migration history, transaction rollback semantics를 명확히 하는 것이다. 자체 storage engine, sharding, replication, custom optimizer는 [ROADMAP.md](ROADMAP.md)에 둔다.
...
DB 백업/복구 surface는 세 의미론을 제공해야 한다.

- consistent snapshot backup
- WAL/archive replay by record count or timestamp
- restore with hash/byte verification and rollback backup

구체 command, archive target, provider credential, crash matrix 상태는 [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md)와 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)를 따른다.
...
|--------|----------|------|--------|---------|
| `@route` | HTTP request/response semantic surface | 요청-응답 | 신뢰 | REST API, 페이지 서빙 |
| `@ws` | WebSocket | 양방향 메시지 | 신뢰 | 실시간 채팅, 알림 |
| `@wt` | WebTransport (H3) | 양방향 스트림/데이터그램 | 선택 | 게임, 미디어, 저지연 |
| `@webrtc` | WebRTC | P2P | 선택 | 미디어 통화, P2P 데이터 |

구체 전송 구현 상태와 프로토콜 우선순위는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)와 [ROADMAP.md](ROADMAP.md)를 따른다.
...
이 장은 목표 최적화 의미론만 정의한다. 현재 compiler artifact, native launcher, client manifest, deploy manifest, verifier delta는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md), [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md), [CHANGELOG.md](CHANGELOG.md)를 따른다.

최적화는 프로그램 의미를 바꾸면 안 된다. optimizer는 HIR, project graph, schema graph, route graph, runtime feature inventory를 입력으로 받아 사용하지 않는 기능을 제거하고, 필요한 runtime surface만 산출물에 남긴다.
...
구체 test runner 구현 상태는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)와 [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md)를 따른다. 이 절은 테스트 문법과 실행 의미만 정의한다.
...
이 장은 `orv.toml`, dependency declaration, lockfile, workspace 의미론을 정의한다. CLI command 구현 범위와 LSP/DAP/dev/build 운영 surface는 [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md)와 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)를 따른다.

### 15.1 orv.toml — 프로젝트 매니페스트

`orv.toml`은 프로젝트 설정 파일이다. Cargo.toml 스타일을 따른다. 최소 `[project].entry`는 source-entry 명령의 entry 파일을 지정한다. `[dependencies]`와 `[dev-dependencies]`는 의존성 요청을 선언하고, lockfile은 이를 exact source/version으로 고정한다. Registry dependency table의 `auth_token_env`는 HTTP/HTTPS registry 요청에 사용할 Bearer token env var 이름이며, secret 값 자체를 manifest나 lockfile에 저장하지 않는다. `[workspace].members`는 workspace member discovery와 cross-member dependency graph의 기준이다.
...
`orv.lock`은 의존성의 정확한 버전과 source identity를 기록하는 잠금 파일이다. 자동 생성되며 버전 관리에 포함한다. Lockfile은 요청된 range, resolved exact version, registry/path source, checksum, optional auth env var name을 deterministic form으로 보존해야 한다. Fetch 구현은 lockfile만 신뢰하며 manifest range를 다시 해석하지 않는다.

### 15.3 CLI 명령어

CLI는 source-entry 명령, package/workspace 명령, build/deploy 검증 명령, editor/LSP/DAP bootstrap 명령으로 나뉜다. 이 사양은 command family의 의미만 정의하고, 전체 command/method 목록과 구현 상태는 [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md)를 따른다.

| Family | Meaning |
|--------|---------|
| source-entry | `run`, `check`, `dump`, `origins`, `graph`, `test` |
| project/package | `init`, `add`, `remove`, `lock`, `fetch` |
| workspace | `workspace new`, `workspace graph`, `workspace lock`, `workspace fetch`, `workspace build` |
| build/deploy | `build`, `verify-build`, `deploy-env-check`, `run-build`, `reveal` |
| DB | `db plan`, `db verify`, `db apply`, `db migrate`, `db rollback`, `db backup`, `db restore`, `db recover`, `db archive` |
| editor/debug | `editor *`, `lsp *`, `dap *` |
...
orv 플랫폼은 자체 에디터를 포함하는 것을 목표로 한다. 이 장은 목표 UX와 graph projection 의미론만 정의한다. 현재 editor snapshot/export, LSP, DAP, trace, native-host artifact 구현 상태와 delta는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md), [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md), [CHANGELOG.md](CHANGELOG.md)를 따른다.

외부 VSCode/Neovim도 LSP/DAP를 통해 지원하되, 자체 에디터는 컴파일러 프로젝트 그래프를 더 직접적으로 소비한다. 따라서 source/production reveal, 도메인 패널, inline runtime value, design edit mode는 first-party editor에서 가장 풍부하게 동작한다.
...
협업 편집은 로드맵이다. 목표 모델은 CRDT 기반 커서/선택/편집 동기화와 프로젝트 그래프 충돌 진단이다. MVP 문서와 기본 에디터 UX에서는 협업 기능을 필수 경로로 보지 않는다.

### 16.11 프로덕션-코드 양방향 추적

구현 상태는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md)를 따른다. 이 절은 목표 동작을 정의한다.

일반적인 개발 흐름은 `코드 작성 -> 실행/프리뷰 확인`이다. orv 에디터는 이 방향을 지원하되, 반대 방향인 **`프로덕션 관찰 -> 코드 reveal -> 프로젝트 그래프 탐색`**을 같은 수준의 기본 기능으로 제공하는 것을 목표로 한다.

핵심 단위는 **origin map**이다. Origin map은 HIR의 실행 가능한 도메인/라우트/응답/호출 노드에 안정적인 origin id와 span fingerprint를 부여하고, source span, graph node, route descriptor, client/server artifact, trace frame을 연결해야 한다.
...
이 표는 target이다. 아직 검증되지 않은 값은 release note나 benchmark report에서 measured claim으로 쓰지 않는다. 각 수치는 하드웨어, 프로젝트 크기, warm/cold 상태, renderer profile, benchmark harness를 함께 기록해야 한다.
...
외부 에디터는 LSP, DAP, Tree-sitter를 통해 지원한다. 세부 method 구현 상태와 날짜형 delta는 [IMPLEMENTATION_MATRIX.md](IMPLEMENTATION_MATRIX.md), [OPERATIONAL_SURFACES.md](OPERATIONAL_SURFACES.md), [CHANGELOG.md](CHANGELOG.md)를 따른다.
