...
> DAP 세부 상태: 현재 external editor bootstrap은 verified nested conditional/hit-condition line, function/data breakpoint, goto/step-in target line, ORV exception filters, reference-runtime debug frame locals, captured function call-stack `stackTrace`, source/module inventory, local-name evaluate/completion, Locals snapshot `setVariable`/`setExpression`, restart, runtime-frame next step-over/stepIn target selection/stepOut/stepBack/restartFrame/goto progression with termination, next line/function/data breakpoint continue/reverseContinue, per-frame stdout/stderr output events, stdio initialized/stopped/continued/terminated events, terminateThreads를 포함한다. 실제 live interpreter pause/resume loop는 아직 없다.
