...
> Dev HMR 상태: `orv dev --hmr`은 `dev/session.json`에 source hash watch set, bundle target, hot-reload/full-reload fallback 전략을 출력한다. 지속 watch loop와 브라우저 HMR transport는 아직 로드맵이다.
...
| `orv verify-build <dir>` | build manifest/plan target, source bundle hash, server runtime/launch artifact, static page zero-runtime shape, client page/JS/WASM metadata와 `orv_start` bootstrap 검증 |
