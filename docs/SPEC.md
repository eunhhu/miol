...
> Native direct launcher delta: generated `server/native/main.rs` validates the native plan and server artifact, then uses the generated route response metadata to choose an execution path. Route artifacts whose handlers are all static JSON, direct route-param JSON objects such as `{ id: @param.id }`, direct query-param JSON objects such as `{ q: @query.q }`, route/query numeric-cast JSON objects such as `{ id: @param.id as int }`, `{ page: @query.page as float }`, or `{ enabled: @param.enabled as bool }`, route/query int literal/captured arithmetic objects such as `{ next: (@param.id as int) + 1 }`, `{ parity: (@query.page as int) % 2 }`, `{ shifted: (@param.id as int) + (@param.offset as int) }`, or `{ next: (@query.page as int) + (@query.step as int) }`, direct request-body JSON passthrough objects such as `{ received: @body }`, direct request-body field JSON objects such as `{ handle: @body.handle }`, request-body field int/float/bool-cast JSON objects plus negated/comparison/logical bool response fields such as `{ active: !(@body.disabled as bool) }`, `{ subscribed: (@body.subscribed as bool) == true }`, `{ matches: (@body.subscribed as bool) == (@query.expected as bool) }`, `{ eligible: (@body.subscribed as bool) && (@query.verified as bool) }`, `{ eligible: (@body.subscribed as bool) || (@query.override as bool) }`, or `{ eligible: !(@body.suspended as bool) && (@query.verified as bool) }`, supported request-body int/float arithmetic JSON objects such as `{ total: (@body.price as float) * (@body.quantity as float) }`, mixed static/domain/arithmetic JSON objects such as `{ kind: "calc", next_id: (@param.id as int) + 1 }`, or simple request-body/route-param/query-param/cross-source guard flows, including numeric comparison guards such as `if (@body.quantity as int) > 0 { @respond ... }`, bool guards such as `if (@body.subscribed as bool) { @respond ... }`, `if !(@body.subscribed as bool) { @respond ... }`, `if (@body.subscribed as bool) == true { @respond ... }`, captured bool conjunction guards such as `if (@body.subscribed as bool) && (@query.verified as bool) { @respond ... }`, captured bool disjunction guards such as `if (@body.subscribed as bool) || (@query.override as bool) { @respond ... }`, or negated captured bool conjunction guards such as `if !(@body.suspended as bool) && (@query.verified as bool) { @respond ... }`, captured integer comparisons such as `if (@body.quantity as int) <= (@body.stock as int) { @respond ... }`, and captured float comparisons such as `if (@body.amount as float) <= (@query.limit as float) { @respond ... }`, bind a dependency-free HTTP/1 listener using `ORV_HOST`, `ORV_PORT`, or the source-backed listen env/default port and dispatch through generated `routes.rs`/`router.rs`/`handlers.rs`; artifacts with DB or otherwise unsupported dynamic route bodies keep the reference `orv run-artifact` bridge so current fullservice behavior remains deployable until broader dynamic native lowering exists.
