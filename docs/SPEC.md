...
> Native editor control command delta: `orv editor export` now records exact runner commands for each debug control in both `state.debug.controls[*].runner_command` and `native-host.json` `debug.control_commands`, so a native host can launch the same `orv editor run-debug ... --control <name>` path that the static shell displays. `orv editor run-debug` now also writes `debug/session-result.html`, a standalone native debug result panel rendering session summary, selected frame, stack frames, executed controls, requested breakpoints, and stopped/all/output events beside the JSON result.
