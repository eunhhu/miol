...
> DAP 세부 상태: `setBreakpoints`는 ProjectGraph selectable node와 AST statement 시작 라인을 합쳐 빈 줄/비실행 라인을 unverified 처리한다. `setExceptionBreakpoints`는 `orv.diagnostics`와 `orv.runtime` filters를 수용한다. `launch`는 reference runtime debug trace를 실행해 HIR statement별 frame, active function call stack, 실제 env locals를 캡처한다. `stackTrace`는 captured function frame hierarchy를 반환한다. `scopes`/`variables`는 project metadata 외에 현재 stopped frame의 runtime-evaluated Locals scope를 제공하고, 함수 호출 결과와 재할당 이후 값도 표시한다. `evaluate`/`completions`도 현재 frame local 이름을 인식한다. `next`/`stepIn`/`stepOut`은 captured runtime frame으로 이동하고 마지막 frame 이후에는 terminated event로 세션을 닫는다. `continue`는 남은 verified breakpoint frame으로 이동하거나 종료한다. `pause`는 현재 line에서 `stopped` event를 내고, `restart`는 현재 program을 재로딩한다. stdio 서버는 stdout/stderr `output` event와 `initialized`, `stopped`, `continued`, `terminated` event frame을 response 뒤에 emit한다.
