...
- `orv db recover --wal db.wal.jsonl --out data.json --until-record 128`, `--until-unix-ms 1770000000000`, 또는 `--until-time 2026-05-02T12:00:00Z` — 현재 구현: JSONL WAL을 record count, unix ms timestamp, 또는 RFC3339 timestamp 경계까지 재생해 `@db.save` 호환 snapshot 복구
...
| `orv db recover` | JSONL WAL을 record count, unix ms timestamp, 또는 RFC3339 timestamp 경계까지 재생해 data snapshot 복구 |
