...
- `@db.connect "memory://local"` — 현재 구현: reference in-memory handle 반환. `@db.connect "file://data/app.wal.jsonl"`은 local WAL-backed handle을 반환한다. `postgres://`/`mysql://`/`sqlite://` 외부 DB 어댑터는 로드맵이며 현재는 명시적으로 실패한다. 기존 자산 이전/상호운용용이며, 프로덕션 기본 경로로는 비권장
...
| `orv init <dir> --name <name> [--template basic\|shop]` | 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, 회원/결제/배송 route scaffold, file-backed local `@payment`/`@shipping` reference adapter record log, `data/shop.wal.jsonl` WAL 지속성, 검증/Compose 배포 README 생성 |
