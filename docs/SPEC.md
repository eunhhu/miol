...
> Deploy env 상태: server prod builds write `deploy/env.example`, link it from `deploy/manifest.json` as `server.env_example`, and require generated runbooks to reference it. The file carries the effective `PORT` assignment plus env/default pairs recorded from DB and commerce adapters such as `SHOP_DATABASE_URL`, `PAYMENT_ADAPTER_URL`, and `SHIPPING_ADAPTER_URL`, so deployment overrides stay source-free while Compose and verifier contracts remain checked.

> Deploy commerce adapter 상태: server prod builds write `deploy/commerce-adapters.json`, link it from `deploy/manifest.json` as `server.commerce_adapters`, and require generated runbooks to reference it. The artifact records payment/shipping adapter kind, mode (`file`, `http`, or env-only), env/default, endpoint or record path, and the reference HTTP POST request contract, and `orv verify-build` rejects drift from the rehydrated source bundle persistence analysis.
