...
> 구현 delta: 서버 build는 `server/runtime-image.json` native runtime image plan과 `server/native/routes.rs` native route table source도 출력한다. image plan은 현재 reference image, planned OCI image tag, native binary path, route/listen/runtime feature shape, `native-codegen`/`native-runtime-image` blocker를 기록하며 `deploy/manifest.json`의 `server.native_runtime_image_plan`에서 참조된다. routes source는 server runtime artifact의 method/path/origin id 목록과 `:param` 및 `:rest*` segment를 인식하고 route params를 `OrvNativeParam`으로 캡처하는 `orv_native_match_route` helper, `orv_native_param_value` lookup helper를 Rust source로 고정하고 `server.native_routes_source`, native plan `routes_source`, reveal production payload에서 참조된다.
