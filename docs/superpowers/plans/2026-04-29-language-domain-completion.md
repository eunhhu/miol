...
**Dev/HMR delta (2026-05-03):** `orv dev --hmr` now emits `dev/session.json` with source hash watch inputs, bundle targets, and hot-reload/full-reload fallback strategy, `orv dev --watch` emits `dev/watch.json` with poll-loop/watch target contract, and `orv dev --watch-loop` runs a poll loop that records `dev/events.json` rebuild/skip events with a manifest transport. `orv verify-build` validates these dev manifests when present. Remaining HMR work is browser/server hot-reload transport.
