...
**DAP delta (2026-05-02):** DAP bootstrap now verifies breakpoints against ProjectGraph selectable nodes plus AST statement lines, accepts ORV diagnostics/runtime exception filters, exposes stopped-line-filtered top-level pure-expression/compound `let`/`const` bindings through a Locals scope, makes visible local names available to `evaluate` and `completions`, advances `next`/`stepIn`/`stepOut` through entry executable lines with last-line termination, supports `restart`, emits stdout/stderr `output` events, and emits stdio `initialized`/`stopped`/`continued`/`terminated` event frames. Remaining DAP work is the real interpreter pause/resume loop with dynamic expression frame locals.
