...
**Package lock/workspace delta (2026-05-03):** `orv lock [dir-or-orv.toml]` now reads `[project]`, `[dependencies]`, and `[dev-dependencies]` metadata from `orv.toml`, writes deterministic JSON `orv.lock` entries with sorted package names, registry/path source metadata, prerelease/build-aware exact versions, local/file/HTTP registry `index.json` resolution for wildcard/caret/tilde/comparator ranges with `requested_version` preservation, and stable `fnv1a64` checksums, and `--check` verifies the committed lockfile without rewriting it. `orv fetch` now verifies that lockfile against the manifest and materializes path/local-registry dependencies plus HTTP registry `source-bundle.json` endpoints into source-bundle cache artifacts and `deps-manifest.json`. `orv add/remove` now edits the selected dependency section and regenerates the lockfile. `orv workspace new` creates a basic member project and records it in root `[workspace].members` with resolver `2`. `orv workspace graph` emits member ProjectGraph/files/dependencies plus path dependency edges as a workspace-wide input artifact. `orv workspace build` now reuses the existing member build pipeline, orders members dependency-first from path dependency edges, verifies each member build directory, writes `workspace-build.json` plus `workspace-graph.json`, and supports `--incremental` skip for unchanged source-bundle input hashes while rebuilding dependents when dependencies rebuild. Remaining package work is HTTPS/authenticated registry transport, disjunction range solving, and richer cross-member dependency semantics.
