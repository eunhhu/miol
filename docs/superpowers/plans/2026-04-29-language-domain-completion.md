...
**DB archive delta (2026-05-03):** `orv db recover` now accepts `--archive <archive.json>` as an alternative to raw `--wal`, resolves file archive targets, and verifies archived WAL hash/byte count before replay. `orv db restore --archive <archive.json> --data <data.json> --at <RFC3339>` now exposes the same hash-verified archive replay as a PITR restore surface. Remaining DB persistence work is remote archive targets, external adapters, and full crash matrix coverage.
...
**Runtime request trace file delta (2026-05-03):** Normal `@server` runtime execution now honors `orv run-artifact/run-build --trace <path>` and `ORV_RUNTIME_REQUEST_TRACE_PATH`, writing the same `orv.production.trace` file on graceful shutdown so run/build/deploy processes have a non-DAP production trace capture path. The reference runtime now also serves `/__orv/trace/events` as an EventSource-compatible trace snapshot when request trace capture is enabled. Remaining trace work is richer native editor consumption and continuous multi-event streaming.
