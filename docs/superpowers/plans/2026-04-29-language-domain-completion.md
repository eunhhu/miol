...
**Package lock/workspace delta (2026-05-03):** `orv lock [dir-or-orv.toml]` now reads `[project]`, `[dependencies]`, and `[dev-dependencies]` metadata from `orv.toml`, writes deterministic JSON `orv.lock` entries with sorted package names, registry/path source metadata, versions, and stable `fnv1a64` checksums, and `--check` verifies the committed lockfile without rewriting it. `orv add/remove` now edits the selected dependency section and regenerates the lockfile. `orv workspace new` creates a basic member project and records it in root `[workspace].members` with resolver `2`. `orv workspace graph` emits member ProjectGraph/files/dependencies plus path dependency edges as a workspace-wide input artifact. `orv workspace build` now reuses the existing member build pipeline, verifies each member build directory, and writes `workspace-build.json` plus `workspace-graph.json` as the top-level workspace build contract. Remaining package work is dependency resolution/download and incremental workspace scheduling.
