...
현재 `orv-compiler`는 HIR의 실행 가능한 도메인/라우트/응답/호출 노드에서 안정적인 origin id, source span fingerprint, traversal 기반 parent-child `contains` edge, call expression에서 resolved function으로 이어지는 `calls` edge를 생성한다. `orv origins <file>`은 이 artifact를 JSON으로 출력한다. `orv reveal <dir> <origin-id>`는 build artifact directory의 origin map, ProjectGraph, server runtime artifact를 읽어 source span, graph node, route descriptor를 JSON으로 반환한다. 프로덕션 trace와 editor reveal을 origin map에 연결하는 단계는 로드맵이다.

### 4.6단계: 초기 build artifact (orv-compiler + orv-cli)

```
HIR + ProjectGraph v1 → build-manifest.json + bundle-plan.json + origin-map.json + project-graph.json + server/app.orv-runtime.json + server/launch.json | pages/index.html | client/app.js | client/app.wasm
```
