...
- Deploy artifacts: `deploy/manifest.json`, `deploy/container.json`, `deploy/compose.yaml`, `deploy/env.example`, `deploy/db-adapters.json`, `deploy/commerce-adapters.json`, `deploy/smoke-test.sh`, `deploy/README.md`
- Verification/reveal surfaces: `orv verify-build`, `orv run-build`, `orv dev`, `orv reveal`, `orv lsp reveal`, `orv editor reveal`

`build-manifest.json`은 HIR origin map에서 추론한 `runtime_features`와 artifact inventory를 기록하고, `bundle-plan.json`은 future bundler target을 선언한다. `source-bundle.json`은 source path/source/content hash snapshot을 보존해 원본 파일 없이도 reveal/LSP reveal과 artifact reanalysis가 source span을 복구하게 한다. Server artifact는 route/listen/runtime/source-bundle 계약을 담고, native plan은 direct-lowered artifact에서 `direct_http` incremental native launcher package와 generated native image Dockerfile/build command를 기록하고, dynamic fallback artifact에서 route table/matcher source, router dispatch source, handler source, runtime image plan, build/run command, `native-codegen`/`native-runtime-image` blocker를 기록한다. Client artifact는 manifest/reactive-plan/JS/WASM path/hash/source-bundle hash와 signal-state/signal-text/signal-attr/signal-event binding 계약으로 current source-bound initial render/state bootstrap/text-slot/attr/event update hook과 future full dynamic/reactive codegen 입력을 분리한다.
...
`server/runtime-image.json`은 reference runtime image, OCI image tag, native binary path, generated Dockerfile/build command, route/listen/runtime feature shape를 같은 schema로 고정한다. `server/native/Dockerfile`은 generated native package를 multi-stage build해서 `/app/server/app`로 배치하고 `ORV_BUILD_DIR=/app` entrypoint로 실행한다. `server/native/routes.rs`는 같은 route descriptors와 contained `@respond` origin ids를 Rust route table and `:param`/`:rest*`-aware matcher, route/query/body/body-field capture, lookup-helper source로 고정하고, `server/native/handlers.rs`는 route/response origin descriptor table, static literal `@respond` JSON body lowering, 1xx/204/304 및 void response empty-body lowering, route-param/query-param JSON object lowering, route/query numeric-cast JSON object lowering such as `{ id: @param.id as int }` or `{ page: @query.page as float }`, route/query int add/sub/mul/div/rem literal and captured-operand lowering such as `{ next: (@param.id as int) + 1 }`, `{ prev: (@query.page as int) - 1 }`, `{ doubled: (@param.id as int) * 2 }`, `{ half: (@query.page as int) / 2 }`, `{ parity: (@param.id as int) % 2 }`, `{ shifted: (@param.id as int) + (@param.offset as int) }`, or `{ next: (@query.page as int) + (@query.step as int) }`, request-body JSON passthrough, request-body field JSON object lowering, request-body field numeric-cast JSON object lowering such as `{ quantity: @body.quantity as int }` or `{ amount: @body.amount as float }`, request-body int add/sub/mul/div/rem lowering such as `{ quantity: (@body.quantity as int) + 1 }`, `{ due: (@body.total as int) - (@body.discount as int) }`, `{ cents: (@body.quantity as int) * 100 }`, `{ share: (@body.total as int) / (@body.parts as int) }`, or `{ remainder: (@body.total as int) % (@body.parts as int) }`, request-body numeric field multiplication such as `{ total: (@body.quantity as int) * (@body.unit_price as int) }` or `{ total: (@body.price as float) * (@body.quantity as float) }`, `{ err: "missing", sku: @body.sku }` 또는 `{ kind: "calc", next_id: (@param.id as int) + 1 }` 같은 mixed static/domain/arithmetic JSON object lowering, `if @body.sku == "" { @respond ... }`, `if @body.token == @query.token { @respond ... }`, `if (@body.quantity as int) > 0 { @respond ... }`, `if (@body.quantity as int) <= (@body.stock as int) { @respond ... }`, `if (@body.amount as float) > 0.0 { @respond ... }`, `if (@body.amount as float) <= (@query.limit as float) { @respond ... }`, `if @param.kind == "sale" { @respond ... }`, 또는 `if @query.mode != "compact" { @respond ... }` 같은 단순 request-body/route/query/cross-source field guard and numeric comparison response flow, dynamic body-lowering placeholder response를 고정한다. `server/native/router.rs`는 route table을 match한 뒤 decoded query params, request body, decoded body fields를 붙이고 handlers source로 dispatch를 위임해 route origin과 first response origin을 함께 싣는 native dispatch contract를 유지한다. `server/native/main.rs`는 generated modules를 링크하고 모든 route handler가 단일 `@respond`이거나 단순 body-field/route-param/query-param/cross-source/numeric-comparison guard 뒤 unconditional respond이며 empty body, static, route-param JSON, query-param JSON, route/query numeric-cast JSON, route/query int literal/captured arithmetic JSON, `{ received: @body }` 같은 request-body JSON passthrough, `{ handle: @body.handle }` 같은 request-body field JSON, `{ quantity: @body.quantity as int }` 또는 `{ amount: @body.amount as float }` 같은 request-body field numeric-cast JSON, `{ quantity: (@body.quantity as int) + 1 }`, `{ due: (@body.total as int) - (@body.discount as int) }`, `{ cents: (@body.quantity as int) * 100 }`, `{ share: (@body.total as int) / (@body.parts as int) }`, `{ remainder: (@body.total as int) % (@body.parts as int) }`, `{ total: (@body.quantity as int) * (@body.unit_price as int) }`, or `{ total: (@body.price as float) * (@body.quantity as float) }` 같은 request-body numeric arithmetic JSON, 또는 mixed static/domain/arithmetic JSON object로 native-lowered 된 artifact는 `std::net::TcpListener` 기반 HTTP/1 loop에서 직접 dispatch한다. DB, unsupported arithmetic expressions outside the supported request-body/route/query slices, unsupported cast 등 아직 native-lowered 되지 않은 dynamic route artifact는 reference `orv run-artifact` bridge로 fallback 한다. `deploy/manifest.json`은 이를 `server.native_runtime_image_plan`, `server.native_routes_source`, `server.native_router_source`, `server.native_handlers_source`로 참조하고, `orv verify-build`는 native plan, image plan, Dockerfile, route/router/handler source가 같은 server artifact를 가리키는지 검증한다.
...
- `orv init <dir> --name <name> [--template basic|shop]` — 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, `GET /catalog` customer catalog, `GET /cart` cart view, `GET /account/sessions` account session view, `GET /admin` 운영 dashboard, `GET /admin/catalog` catalog read model, route scaffold, 검증/Compose 배포 README 생성
...
- `orv editor snapshot/reveal/runtime/debug/run-debug/export/trace/trace-stream` — first-party editor bootstrap JSON, source-hash watch set, build-origin navigation payload, runtime inspection pane JSON, DAP control transport/result JSON plus result HTML panel, static editor shell artifact with ProjectGraph/panel-list/runtime-frame/trace-detail rendering and optional trace state, native-host DAP configuration/control/breakpoint inventory, trace status filter/frame navigation inventory, captured request trace navigation payload와 EventSource snapshot/frame consumption payload 출력
