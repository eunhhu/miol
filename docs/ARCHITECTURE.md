...
| `orv-cli` | CLI 프론트엔드. 현재 `init`, `run`, `dev`, `check`, `dump`, `origins`, `graph`, `test`, `lsp snapshot/reveal/serve --stdio`, `dap serve --stdio`, `build`, `build --prod`, `verify-build`, `verify-artifact`, `check-artifact`, `run-artifact`, `run-build`, `reveal`, `db plan/verify/apply/migrate/rollback/backup/restore/recover/squash`으로 프로젝트 scaffold, 로드/해석/분석/실행, origin map 출력, ProjectGraph v1 출력, test block runner bootstrap, editor bootstrap snapshot, LSP-style production reveal, stdio LSP initialize/shutdown/documentSymbol/codeLens/codeAction/documentLink/foldingRange/selectionRange/semanticTokens/diagnostic/workspaceDiagnostic/executeCommand/definition/references/documentHighlight/prepareRename/rename/hover/completion/workspaceSymbol와 didOpen/didChange open-buffer cache, stdio DAP  bootstrap, `orv.toml` `[project].entry` source-entry resolution, 초기 build artifact 및 prod deploy manifest 출력/검증/재분석/reference 실행, build+verify+run-build dev bootstrap, DB schema/data snapshot workflow를 오케스트레이션 | orv-core, orv-diagnostics, orv-syntax, orv-resolve, orv-analyzer, orv-compiler, orv-project, orv-runtime, clap |

DAP bootstrap은 `orv-cli` 안에서 프로젝트 로더와 AST/ProjectGraph를 재사용한다. `setBreakpoints`는 nested AST statement line까지 executable line으로 verified 처리하고 Locals 기반 condition/hitCondition을 평가한다. `setFunctionBreakpoints`는 captured runtime call-stack 함수 이름에 매칭하며, `dataBreakpointInfo`/`setDataBreakpoints`는 현재 Locals 변수의 값 변경 frame에 매칭한다. `gotoTargets`는 sourceReference+line target id를 만든다. `setExceptionBreakpoints`는 ORV diagnostics/runtime filters를 수용한다. `launch`는 `orv-runtime`의 debug trace로 실행된 HIR statement frame, active function call stack, 실제 env locals, stdout delta를 캡처하며, `loadedSources`/`modules`는 같은 프로젝트 파일 inventory를 source 또는 module shape로 노출한다. `scopes`/`variables`/`evaluate`/`completions`는 현재 frame의 runtime value를 읽는다. `setVariable`/`setExpression`은 현재 frame Locals snapshot 값을 갱신한다. `stackTrace`는 captured function frame hierarchy를 반환한다. `next`/`stepIn`은 captured runtime frame list를 따라 stopped line을 갱신하고 frame stdout을 output event로 flush하며, `stepOut`은 caller frame으로 빠져나가고, 마지막 frame 이후 session을 종료한다. `stepBack`은 직전 captured frame으로, `reverseContinue`는 이전 line/function/data breakpoint frame으로 이동한다. `goto`는 target id에 대응하는 captured runtime frame으로 이동한다. `continue`는 남은 line/function/data breakpoint frame으로 이동하거나 종료한다. `restart`는 현재 launch program을 재로딩하고, stdio transport는 response 뒤에 output/lifecycle event frame을 flush한다.
...
현재 런타임은 HIR을 직접 평가한다. 일반 표현식, 함수, 타입/캐스트, HTML 값, 서버 라우트, 인메모리 `@db`, 명시적 `@db.save/load` JSON snapshot, `@db.wal(path)` JSONL append+fsync WAL replay, `@db.checkpoint()` WAL snapshot compaction, `@db.savepoint()`/`@db.rollback(point)` 메모리 상태 복원, 정적 파일 `@serve`, 그리고 일부 고급 도메인의 reference stub을 실행한다. CLI `orv db recover`는 WAL을 complete record 경계까지 재생해 `@db.save` 호환 snapshot으로 복구한다. `@server`는 tokio current-thread 런타임과 hyper HTTP/1.1 서버를 사용하며, 매칭된 route의 origin id를 `x-orv-origin-id` 응답 헤더에 싣는다.
