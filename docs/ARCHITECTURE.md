...
- `orv lock [dir-or-orv.toml] [--check]` — `orv.toml` project/dependency metadata와 local/file/HTTP registry `index.json` 기반 prerelease/build-aware exact/wildcard/caret/tilde/comparator/disjunction range 결과, optional `auth_token_env` 이름을 deterministic `orv.lock` JSON으로 고정하거나 최신성 검증
