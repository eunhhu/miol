...
DAP bootstrap은 `orv-cli` 안에서 프로젝트 로더와 AST/ProjectGraph를 재사용한다. `setBreakpoints`는 nested AST statement line까지 executable line으로 verified 처리하고 Locals 기반 condition/hitCondition을 평가한다. `setFunctionBreakpoints`는 captured runtime call-stack 함수 이름에 매칭하며, `dataBreakpointInfo`/`setDataBreakpoints`는 현재 Locals 변수의 값 변경 frame에 매칭한다. `gotoTargets`는 sourceReference+line target id를 만든다. `setExceptionBreakpoints`는 ORV diagnostics/runtime filters를 수용한다. `launch`는 `orv-runtime`의 debug trace로 실행된 HIR statement frame, active function call stack, 실제 env locals, stdout delta를 캡처하며, `loadedSources`/`modules`는 같은 프로젝트 파일 inventory를 source 또는 module shape로 노출한다. `scopes`/`variables`/`evaluate`/`completions`는 현재 frame의 runtime value를 읽는다. `setVariable`/`setExpression`은 현재 frame Locals snapshot 값을 갱신한다. `stackTrace`는 captured function frame hierarchy를 반환한다. `next`/`stepIn`/`stepOut`은 captured runtime frame list를 따라 stopped line을 갱신하고 frame stdout을 output event로 flush하며, 마지막 frame 이후 session을 종료한다. `stepBack`은 직전 captured frame으로, `reverseContinue`는 이전 line/function/data breakpoint frame으로 이동한다. `goto`는 target id에 대응하는 captured runtime frame으로 이동한다. `continue`는 남은 line/function/data breakpoint frame으로 이동하거나 종료한다. `restart`는 현재 launch program을 재로딩하고, stdio transport는 response 뒤에 output/lifecycle event frame을 flush한다.
