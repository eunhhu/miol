...
- `orv init <dir> --name <name> [--template basic|shop]` — 최소 프로젝트 또는 쇼핑몰 `GET /` HTML form 홈, `GET /catalog` customer catalog, `GET /account/sessions` account session view, `GET /admin` 운영 dashboard, `GET /admin/catalog` catalog read model, route scaffold, 검증/Compose 배포 README 생성
