...
- `orv editor snapshot/reveal/runtime/export/trace` — first-party editor bootstrap JSON, source-hash watch set, build-origin navigation payload, runtime inspection pane JSON, static editor shell artifact with panel-list rendering and optional trace state, captured request trace navigation payload 출력
