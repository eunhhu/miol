...
- `orv fetch [dir-or-orv.toml] --out <dir>` — 최신 lockfile에서 path/local registry dependency source-bundle cache와 `deps-manifest.json` 출력
...
- `orv workspace build [root] --out <dir> [--prod]` — path dependency edge 기반 dependency-first 순서로 각 workspace member를 기존 build pipeline으로 빌드/검증하고 top-level workspace build manifest 출력
