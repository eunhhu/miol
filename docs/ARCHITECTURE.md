...
DAP bootstrap은 `orv-cli` 안에서 프로젝트 로더와 AST/ProjectGraph를 재사용한다. `setBreakpoints`는 executable line만 verified 처리하고, `setExceptionBreakpoints`는 ORV diagnostics/runtime filters를 수용한다. `scopes`/`variables`/`evaluate`/`completions`는 stopped line 이하의 top-level pure-expression/compound locals를 같은 세션 state에서 읽는다. `next`/`stepIn`/`stepOut`은 entry executable line list를 따라 stopped line을 갱신하고 마지막 line 이후 session을 종료한다. `restart`는 현재 launch program을 재로딩하고, stdio transport는 response 뒤에 output/lifecycle event frame을 flush한다.
