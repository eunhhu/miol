...
| `orv-cli` | CLI 프론트엔드. 현재 `init`, `run`, `dev`, `check`, `dump`, `origins`, `graph`, `test`, `lsp snapshot/reveal/serve --stdio`, `dap serve --stdio`, `build`, `verify-build`, `verify-artifact`, `check-artifact`, `run-artifact`, `run-build`, `reveal`, `db plan/apply/migrate/rollback`으로 프로젝트 scaffold, 로드/해석/분석/실행, origin map 출력, ProjectGraph v1 출력, test block runner bootstrap, editor bootstrap snapshot, LSP-style production reveal, stdio LSP initialize/shutdown/documentSymbol/codeLens/codeAction/documentLink/foldingRange/selectionRange/semanticTokens/diagnostic/workspaceDiagnostic/executeCommand/definition/references/documentHighlight/prepareRename/rename/hover/completion/workspaceSymbol와 didOpen/didChange open-buffer cache, stdio DAP initialize/launch/configurationDone/setBreakpoints/breakpointLocations/exceptionInfo/threads/stackTrace/scopes/variables/evaluate/completions/loadedSources/source/continue/step/disconnect/terminate bootstrap, `orv.toml` `[project].entry` source-entry resolution, 초기 build artifact 출력/검증/재분석/reference 실행, build+verify+run-build dev bootstrap, DB schema snapshot workflow를 오케스트레이션 | orv-core, orv-diagnostics, orv-syntax, orv-resolve, orv-analyzer, orv-compiler, orv-project, orv-runtime, clap |

DAP bootstrap은 `orv-cli` 안에서 프로젝트 로더와 AST/ProjectGraph를 재사용한다. `setBreakpoints`는 executable line만 verified 처리하고, `setExceptionBreakpoints`는 ORV diagnostics/runtime filters를 수용한다. `launch`는 `orv-runtime`의 debug trace로 실행된 HIR statement frame, active function call stack, 실제 env locals, stdout delta를 캡처하며, `scopes`/`variables`/`evaluate`/`completions`는 현재 frame의 runtime value를 읽는다. `stackTrace`는 captured function frame hierarchy를 반환한다. `next`/`stepIn`/`stepOut`은 captured runtime frame list를 따라 stopped line을 갱신하고 frame stdout을 output event로 flush하며, 마지막 frame 이후 session을 종료한다. `continue`는 남은 verified breakpoint frame으로 이동하거나 종료한다. `restart`는 현재 launch program을 재로딩하고, stdio transport는 response 뒤에 output/lifecycle event frame을 flush한다.
