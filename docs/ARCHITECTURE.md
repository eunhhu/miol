...
- `orv lock [dir-or-orv.toml] [--check]` — `orv.toml` project/dependency metadata와 local/file/HTTP registry `index.json` 기반 exact/`*`/caret range 결과를 deterministic `orv.lock` JSON으로 고정하거나 최신성 검증
