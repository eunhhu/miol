...
- `orv dev <file-or-orv.toml> --out <dir> [--hmr]` — build + verify-build + run-build reference dev bootstrap, optional HMR session manifest

로드맵 커맨드:
- native 서버 바이너리 + 실제 클라이언트 WASM/JS 코드젠/글루 번들 빌드
- `orv dev --watch` — 변경 감지 loop와 HMR 개발 서버
