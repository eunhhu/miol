...
`server/runtime-image.json`은 reference runtime image, planned OCI image tag, native binary path, route/listen/runtime feature shape를 같은 schema로 고정한다. `server/native/routes.rs`는 같은 route descriptors와 contained `@respond` origin ids를 Rust route table and `:param`/`:rest*`-aware matcher, route/query/body capture, lookup-helper source로 고정하고, `server/native/handlers.rs`는 route/response origin descriptor table, static literal `@respond` JSON body lowering, route-param/query-param JSON object lowering, request-body JSON passthrough, dynamic body-lowering placeholder response를 고정한다. `server/native/router.rs`는 route table을 match한 뒤 decoded query params와 request body를 붙이고 handlers source로 dispatch를 위임해 route origin과 first response origin을 함께 싣는 native dispatch contract를 유지한다. `server/native/main.rs`는 generated modules를 링크하고 모든 route handler가 static, route-param JSON, query-param JSON, 또는 `{ received: @body }` 같은 request-body JSON passthrough로 native-lowered 된 artifact는 `std::net::TcpListener` 기반 HTTP/1 loop에서 직접 dispatch하며, 아직 native-lowered 되지 않은 `@body.field`/DB 등 dynamic route artifact는 reference `orv run-artifact` bridge로 fallback 한다. `deploy/manifest.json`은 이를 `server.native_runtime_image_plan`, `server.native_routes_source`, `server.native_router_source`, `server.native_handlers_source`로 참조하고, `orv verify-build`는 native plan, image plan, route/router/handler source가 같은 server artifact를 가리키는지 검증한다.
