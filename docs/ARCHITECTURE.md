...
현재 런타임은 HIR을 직접 평가한다. 일반 표현식, 함수, 타입/캐스트, HTML 값, 서버 라우트, 인메모리 `@db`, 명시적 `@db.save/load` JSON snapshot, `@db.wal(path)` JSONL append+fsync WAL replay, `@db.checkpoint()` WAL snapshot compaction, `@db.savepoint()`/`@db.rollback(point)` 메모리 상태 복원, 정적 파일 `@serve`, 그리고 일부 고급 도메인의 reference stub을 실행한다. `@server`는 tokio current-thread 런타임과 hyper HTTP/1.1 서버를 사용하며, 매칭된 route의 origin id를 `x-orv-origin-id` 응답 헤더에 싣는다.
