...
| `orv-cli` | CLI 프론트엔드. 현재 `init`, `run`, `dev`, `check`, `dump`, `origins`, `graph`, `test`, `lsp snapshot/reveal/serve --stdio`, `dap serve --stdio`, `build`, `verify-build`, `verify-artifact`, `check-artifact`, `run-artifact`, `run-build`, `reveal`, `db plan/apply/migrate/rollback/backup/restore`으로 프로젝트 scaffold, 로드/해석/분석/실행, origin map 출력, ProjectGraph v1 출력, test block runner bootstrap, editor bootstrap snapshot, LSP-style production reveal, stdio LSP initialize/shutdown/documentSymbol/codeLens/codeAction/documentLink/foldingRange/selectionRange/semanticTokens/diagnostic/workspaceDiagnostic/executeCommand/definition/references/documentHighlight/prepareRename/rename/hover/completion/workspaceSymbol와 didOpen/didChange open-buffer cache, stdio DAP  bootstrap, `orv.toml` `[project].entry` source-entry resolution, 초기 build artifact 출력/검증/재분석/reference 실행, build+verify+run-build dev bootstrap, DB schema/data snapshot workflow를 오케스트레이션 | orv-core, orv-diagnostics, orv-syntax, orv-resolve, orv-analyzer, orv-compiler, orv-project, orv-runtime, clap |

DAP bootstrap은 `orv-cli` 안에서 프로젝트 로더와 AST/ProjectGraph를 재사용한다. `setBreakpoints`는 executable line만 verified 처리하고, `gotoTargets`는 sourceReference+line target id를 만든다. `setExceptionBreakpoints`는 ORV diagnostics/runtime filters를 수용한다. `launch`는 `orv-runtime`의 debug trace로 실행된 HIR statement frame, active function call stack, 실제 env locals, stdout delta를 캡처하며, `loadedSources`/`modules`는 같은 프로젝트 파일 inventory를 source 또는 module shape로 노출한다. `scopes`/`variables`/`evaluate`/`completions`는 현재 frame의 runtime value를 읽는다. `setVariable`/`setExpression`은 현재 frame Locals snapshot 값을 갱신한다. `stackTrace`는 captured function frame hierarchy를 반환한다. `next`/`stepIn`/`stepOut`은 captured runtime frame list를 따라 stopped line을 갱신하고 frame stdout을 output event로 flush하며, 마지막 frame 이후 session을 종료한다. `stepBack`은 직전 captured frame으로, `reverseContinue`는 이전 verified breakpoint frame으로 이동한다. `goto`는 target id에 대응하는 captured runtime frame으로 이동한다. `continue`는 남은 verified breakpoint frame으로 이동하거나 종료한다. `restart`는 현재 launch program을 재로딩하고, stdio transport는 response 뒤에 output/lifecycle event frame을 flush한다.
...
로드맵 커맨드:
- `orv build --prod` — 서버 바이너리 + 클라이언트 WASM/JS 번들 빌드
