...
orv는 Rust workspace로 구성된 10개 크레이트의 파이프라인 아키텍처를 따른다. 현재 구현은 `.orv` 소스를 로드/파싱/해석/분석한 뒤 HIR을 레퍼런스 tree-walking 런타임으로 실행하는 MVP다. `orv-compiler`는 HIR 기반 origin map과 build manifest artifact를 생성할 수 있고, `@server` 런타임은 매칭된 route origin id를 HTTP 응답 헤더로 노출한다. `orv init`은 최소 `orv.toml`/`src/main.orv` 프로젝트 또는 `--template shop` 쇼핑몰 route scaffold를 만들고, source-entry CLI는 단일 파일뿐 아니라 `orv.toml` `[project].entry`와 프로젝트 디렉터리 입력도 받는다. `orv build`는 현재 manifest/origin-map/project-graph/source-bundle/source-bundled server runtime artifact JSON과 reference server launch artifact 디렉터리를 만들며, HTML-only entry는 zero-runtime `pages/index.html` 정적 페이지도 출력한다. `let sig` 또는 HTML await가 필요한 client entry는 non-zero-runtime `pages/index.html` shell, source-bundle/wasm target metadata를 노출하는 `client/app.js` bootstrap, `orv.client` custom section에 source-bundle metadata를 담은 `client/app.wasm` placeholder로 client bundle 계약을 고정한다. `orv build --prod`는 여기에 `deploy/manifest.json`과 `deploy/server.sh` reference server entrypoint를 추가한다. CLI는 build directory target, deploy manifest, server/client/source-bundle artifact 검증, artifact 재분석, reference runtime 실행, build directory launch 실행, build artifact origin reveal을 할 수 있다. native 서버 바이너리와 실제 클라이언트 WASM/JS 코드젠은 아직 구현되지 않은 컴파일러 로드맵이다.
...
| `orv-cli` | CLI 프론트엔드. 현재 `init`, `run`, `dev`, `check`, `dump`, `origins`, `graph`, `test`, `lsp snapshot/reveal/serve --stdio`, `dap serve --stdio`, `build`, `build --prod`, `verify-build`, `verify-artifact`, `check-artifact`, `run-artifact`, `run-build`, `reveal`, `db plan/verify/apply/migrate/rollback/backup/restore/recover/archive/squash`으로 프로젝트 scaffold, 로드/해석/분석/실행, origin map 출력, ProjectGraph v1 출력, test block runner bootstrap, editor bootstrap snapshot, LSP-style production reveal, stdio LSP initialize/shutdown/documentSymbol/codeLens/codeAction/documentLink/foldingRange/selectionRange/semanticTokens/diagnostic/workspaceDiagnostic/executeCommand/definition/references/documentHighlight/prepareRename/rename/hover/completion/workspaceSymbol와 didOpen/didChange open-buffer cache, stdio DAP  bootstrap, `orv.toml` `[project].entry` source-entry resolution, 초기 build artifact 및 prod deploy manifest 출력/검증/재분석/reference 실행, build+verify+run-build dev bootstrap, DB schema/data snapshot workflow를 오케스트레이션 | orv-core, orv-diagnostics, orv-syntax, orv-resolve, orv-analyzer, orv-compiler, orv-project, orv-runtime, clap |

DAP bootstrap은 `orv-cli` 안에서 프로젝트 로더와 AST/ProjectGraph를 재사용한다. `setBreakpoints`는 source path 또는 loaded sourceReference를 받아 nested AST statement line까지 executable line으로 verified 처리하고 Locals 기반 condition/hitCondition을 평가한다. `setFunctionBreakpoints`는 captured runtime call-stack 함수 이름에 매칭하며, `dataBreakpointInfo`/`setDataBreakpoints`는 현재 Locals 변수의 값 변경 frame에 매칭한다. `gotoTargets`는 sourceReference+line target id를 만든다. `stepInTargets`는 현재 runtime frame에서 직접 진입 가능한 captured function call target id를 만든다. `setExceptionBreakpoints`는 ORV diagnostics/runtime filters를 수용한다. `launch`는 `orv-runtime`의 debug trace로 실행된 HIR statement frame, active function call stack, 실제 env locals, stdout delta를 캡처하며, `loadedSources`/`modules`는 같은 프로젝트 파일 inventory를 source 또는 module shape로 노출한다. `scopes`/`variables`/`evaluate`/`completions`는 현재 frame의 runtime value를 읽는다. `setVariable`/`setExpression`은 현재 frame Locals snapshot 값을 갱신한다. `stackTrace`는 captured function frame hierarchy를 반환한다. `next`는 같은 call-stack 깊이의 다음 frame으로 step-over하고, `stepIn`은 targetId가 있으면 선택된 function call frame으로, 없으면 다음 captured runtime frame으로 진입하며, `stepOut`은 caller frame으로 빠져나가고, 마지막 frame 이후 session을 종료한다. `stepBack`은 직전 captured frame으로, `restartFrame`은 현재 captured function frame의 첫 runtime frame으로, `reverseContinue`는 이전 line/function/data breakpoint frame으로 이동한다. `goto`는 target id에 대응하는 captured runtime frame으로 이동한다. `continue`는 남은 line/function/data breakpoint frame으로 이동하거나 종료한다. `restart`는 현재 launch program을 재로딩하고, stdio transport는 response 뒤에 output/lifecycle event frame을 flush한다.
...
현재 런타임은 HIR을 직접 평가한다. 일반 표현식, 함수, 타입/캐스트, HTML 값, 서버 라우트, 인메모리 `@db`, 명시적 `@db.save/load` JSON snapshot, `@db.wal(path)` JSONL append+fsync WAL replay with `ts_unix_ms` record timestamps, `@db.checkpoint()` WAL snapshot compaction, `@db.savepoint()`/`@db.rollback(point)` 메모리 상태 복원, 정적 파일 `@serve`, 그리고 일부 고급 도메인의 reference stub을 실행한다. CLI `orv db recover`는 WAL을 complete record count, unix ms timestamp, 또는 RFC3339 timestamp 경계까지 재생해 `@db.save` 호환 snapshot으로 복구하고, `orv db archive`는 WAL record/timestamp/hash manifest를 생성하며 `--target file://...`이면 WAL과 manifest를 file archive target으로 복사한다. `@server`는 tokio current-thread 런타임과 hyper HTTP/1.1 서버를 사용하며, 매칭된 route의 origin id를 `x-orv-origin-id` 응답 헤더에 싣는다.
...
서버 네이티브 바이너리와 실제 클라이언트 WASM/JS 코드젠은 아직 구현되어 있지 않다. 현재는 `let sig` 또는 client-side HTML await가 필요한 entry에서 page shell, `ORV_CLIENT_BOOTSTRAP` metadata JS bootstrap, `orv.client` custom section에 source-bundle metadata를 담은 유효 WASM placeholder인 `client/app.wasm`을 출력해 bundle/verify/deploy 계약을 먼저 고정한다.
...
- `orv init <dir> --name <name> [--template basic|shop]` — 최소 프로젝트 또는 쇼핑몰 route scaffold 생성
...
- `orv verify-build <dir>` — build manifest/plan target 존재, source bundle hash, server artifact, static page zero-runtime shape, client page/JS/WASM bootstrap 검증
