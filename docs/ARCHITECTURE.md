...
DAP bootstrap은 `orv-cli` 안에서 프로젝트 로더와 AST/ProjectGraph를 재사용한다. `setBreakpoints`는 source path 또는 loaded sourceReference를 받아 nested AST statement line까지 executable line으로 verified 처리하고 Locals 기반 condition/hitCondition을 평가한다. `setFunctionBreakpoints`는 captured runtime call-stack 함수 이름에 매칭하며, `dataBreakpointInfo`/`setDataBreakpoints`는 현재 Locals 변수의 값 변경 frame에 매칭한다. `gotoTargets`는 sourceReference+line target id를 만든다. `stepInTargets`는 현재 runtime frame에서 직접 진입 가능한 captured function call target id를 만든다. `setExceptionBreakpoints`는 ORV diagnostics/runtime filters를 수용한다. 기본 `launch`는 `orv-runtime`의 debug trace로 실행된 HIR statement frame, active function call stack, 실제 env locals, stdout delta를 캡처하며, `launch.arguments.live=true`는 `DebugStepper`로 첫 frame까지만 실행한 뒤 `next`/`stepIn`/`stepOut`/`continue`에서 interpreter state를 계속 전진시킨다. `loadedSources`/`modules`는 같은 프로젝트 파일 inventory를 source 또는 module shape로 노출한다. `scopes`/`variables`/`evaluate`/`completions`는 현재 frame의 runtime value를 읽는다. `setVariable`/`setExpression`은 현재 frame Locals snapshot 값을 갱신한다. `stackTrace`는 captured function frame hierarchy를 반환한다. `next`는 같은 call-stack 깊이의 다음 frame으로 step-over하고, `stepIn`은 targetId가 있으면 선택된 function call frame으로, 없으면 다음 captured runtime frame 또는 live runtime frame으로 진입하며, `stepOut`은 caller frame으로 빠져나가고, 마지막 frame 이후 session을 종료한다. `stepBack`은 직전 captured frame으로, `restartFrame`은 현재 captured function frame의 첫 runtime frame으로, `reverseContinue`는 이전 line/function/data breakpoint frame으로 이동한다. `goto`는 target id에 대응하는 captured runtime frame으로 이동한다. `continue`는 남은 line/function/data breakpoint frame으로 이동하거나, live mode에서는 다음 breakpoint까지 실제 stepper를 실행하고 종료 시 terminated event를 낸다. `restart`는 현재 launch program을 재로딩하고, stdio transport는 response 뒤에 output/lifecycle event frame을 flush한다.
