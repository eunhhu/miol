...
서버 네이티브 바이너리와 실제 클라이언트 WASM/JS 코드젠은 아직 구현되어 있지 않다. 현재는 `let sig` 또는 client-side HTML await가 필요한 entry에서 page shell, JS loader, 유효한 최소 WASM module인 `client/app.wasm`을 출력해 bundle/verify/deploy 계약을 먼저 고정한다.
